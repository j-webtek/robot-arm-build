...
PASS command: `python scripts/sign_off_step.py --step 14 --status PASS --operator "OPERATOR_NAME"` (add `--witness "WITNESS_NAME"` when required).

HOLD command: `python scripts/sign_off_step.py --step 14 --status HOLD --operator "OPERATOR_NAME" --hold "REASON"` (repeat `--hold` for separate reasons).
