# Step 09 — Finish This Step and Continue

Do not continue unless every mandatory acceptance item is PASS and there is no unresolved HOLD.

## Required handoff results

- Required result: Phone pose and cable routing pass ten cycles.
- Required result: Both soft tips make gentle contact only.
- Required result: The device remains loaded for the explicit no-go inspection in Step 10.
...
- Required result: The responsible operator ran `python scripts/sign_off_step.py --step 09 --status PASS --operator "OPERATOR_NAME"`; the helper validated and completed `signoff.json`.
...
Step 10 cannot be opened from this handoff until Step 09 is computed `COMPLETE`.
