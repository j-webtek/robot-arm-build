...
def load_vision_dependencies() -> None:
    """Load optional native vision packages after argparse handles --help."""
    global cv2, np
    errors = io.StringIO()
    try:
        ...
        raise SystemExit(
            ...
            "isolated environment and install requirements-vision.txt before "
            f"calibration. Original error: {exc}"
        ) from None
    if not hasattr(cv2_module, "aruco"):
        raise SystemExit(
            "The installed OpenCV build has no aruco module; install "
            "opencv-contrib-python from requirements-vision.txt."
        )
