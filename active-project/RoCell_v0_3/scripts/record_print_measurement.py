...
def has_blank(value) -> bool:
    ...
    if isinstance(value, dict):
        return not value or any(has_blank(item) for item in value.values())
    if isinstance(value, list):
        return not value or any(has_blank(item) for item in value)
    return False
...
def main() -> None:
    ...
    # Validate the complete proposed record before replacing the canonical file.
    # This keeps a malformed PASS (including typed engineering-limit gates) from
    # being persisted merely because output regeneration fails afterward.
    jobs_config = json.loads((ROOT / "config" / "print_jobs.json").read_text(encoding="utf-8"))
    try:
        readiness.validate_measurement_record(data, jobs_config)
