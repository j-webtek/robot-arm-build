...
def main() -> None:
    ...
    parser.add_argument("--running-title", default="RoCell RC03-INT-R1 - Assembly Manual")
    ...
    # Synchronization markers belong in the controlled Markdown source but are
    # implementation metadata, not operator-facing manual content.
    source_text = re.sub(
        r"(?m)^\s*<!--\s*(?:BEGIN|END) AUTO-GENERATED: [A-Z0-9_]+\s*-->\s*\n?",
        "",
        source_text,
    )
    body_html = markdown(source_text)
