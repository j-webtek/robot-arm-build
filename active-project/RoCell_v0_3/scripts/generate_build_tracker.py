...
def unresolved_job_prerequisites(
    job: dict[str, Any],
    gates: dict[str, Any],
) -> list[str]:
    """Return gates that do not release this exact print job.

    Most gates are scalar PASS/FAIL records.  QIDI Studio evidence is an
    aggregate record indexed by job ID, so it must be evaluated with the
    per-job predicate shared with the print-readiness report.
    """
    missing: list[str] = []
    for gate_id in job["prerequisites"]:
        if gate_id == "qidi_studio_roundtrip_confirmed":
            if not readiness.qidi_job_qualified(gates[gate_id], job):
...
def build_tracker() -> dict[str, Any]:
    ...
    for job in jobs:
        ...
        # Keep lifecycle readiness identical to PRINT_READINESS, including
        # job-specific native QIDI Studio qualification.
        missing_prerequisites = unresolved_job_prerequisites(job, gates)
        qidi_required = "qidi_studio_roundtrip_confirmed" in job["prerequisites"]
        if not qidi_required:
            qidi_status = "NOT_REQUIRED"
        elif "qidi_studio_roundtrip_confirmed" in missing_prerequisites:
            qidi_status = "MISSING"
        else:
            qidi_status = "QUALIFIED"
        ...
        rows.append(
            {
                ...
                "native_qidi_project_status": qidi_status,
    ...
    return {
        ...
        "summary": {
            ...
            "qidi_qualified_jobs": sum(
                row["native_qidi_project_status"] == "QUALIFIED" for row in rows
            ),
...
def write_outputs(report: dict[str, Any]) -> None:
    ...
    lines = [
        ...
        "Machine jobs require native QIDI Studio evidence for that exact job. "
    ...
    for row in report["jobs"]:
        ...
        lines.append(
            ...
            f"{row['recorded_state']} | {row['native_qidi_project_status']} | "
            f"{row['stage']} | {row['kit_id']} | "
