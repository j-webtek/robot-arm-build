"""Offline-only proposed lateral multi-joint hover cycle from the r83 endpoint."""
from __future__ import annotations

import hashlib
import math
from pathlib import Path

from rocell.geometry import UrdfModel

from .air_typing_continuation_preview import _angles
from .air_typing_r83_review import EXPORTS as R83_EXPORTS
from .large_pose_ladder import _sample
from .product_ghost_export_review import _read


SOURCE_GOALS = (2047, 2075, 2039, 2620, 2233, 2040, 2047)
SOURCE_POSITIONS = (2041, 2082, 2033, 2622, 2233, 2041, 2047)
A = (2047, 2075, 2039, 2600, 2233, 2040, 2047)
LATERAL_HOVER = (1994, 2093, 2021, 2618, 2197, 2040, 2047)
TARGETS = (A, LATERAL_HOVER, A, LATERAL_HOVER, A)
PHASES = ("A_source", "lateral_hover_1", "A_return_1", "lateral_hover_2", "A_return_2")


def review_source(export_root):
    row, digest = _read(Path(export_root).resolve(), R83_EXPORTS[-1],
                        "attachment-elbow-grid-assessment.json")
    if (row.get("status") != "ELBOW_GRID_LEG_ENDPOINT_VERIFIED"
            or row.get("source_kind") != "controller_feedback"
            or row.get("leg") != 16
            or row.get("final_goals") != list(SOURCE_GOALS)
            or row.get("final_positions") != list(SOURCE_POSITIONS)):
        raise ValueError("Retained r83 source differs")
    return digest


def validate_recipe():
    if TARGETS != (A, LATERAL_HOVER, A, LATERAL_HOVER, A):
        raise ValueError("Proposed hover sequence differs")
    prior = SOURCE_GOALS
    for leg, target in enumerate(TARGETS, 1):
        changed = [i for i in range(7) if target[i] != prior[i]]
        expected = [3] if leg == 1 else [0, 1, 2, 3, 4]
        if (changed != expected or max(abs(target[i] - prior[i]) for i in changed) > 60
                or any(abs(target[i] - prior[i]) < 10 for i in changed)):
            raise ValueError("Unreviewed multi-joint hover step")
        prior = target
    return True


def preview(model_path):
    validate_recipe()
    path = Path(model_path).resolve(strict=True)
    model = UrdfModel.from_file(path)
    previous = SOURCE_POSITIONS
    goals = SOURCE_GOALS
    rows = []
    for leg, target in enumerate(TARGETS, 1):
        min_z = min_proxy = math.inf
        for step in range(101):
            fraction = step / 100
            counts = tuple(a + fraction * (b - a) for a, b in zip(previous, target))
            angles = _angles(counts)
            if not (-math.pi <= angles[0] <= math.pi and
                    -math.pi/2 <= angles[1] <= math.pi/2 and
                    0 <= angles[2] <= 2.95 and
                    -math.pi/2 <= angles[3] <= math.pi/2):
                raise ValueError("Provisional joint bound")
            points, proxy = _sample(model, angles)
            min_z = min(min_z, points["hand_tcp"][2])
            min_proxy = min(min_proxy, proxy)
        if min_z < 40 or min_proxy < 30:
            raise ValueError("Modeled clearance screen failed")
        rows.append(dict(leg=leg, phase=PHASES[leg-1], goals=list(target),
                         selected_joints=[i for i in range(7) if target[i] != goals[i]],
                         maximum_goal_step_counts=max(abs(a-b) for a, b in zip(goals, target)),
                         minimum_modeled_tcp_z_mm=min_z,
                         minimum_modeled_link_axis_separation_mm=min_proxy))
        previous = target
        goals = target
    return dict(schema="rocell.air_typing_multi_hover_preview.v1",
                status="OFFLINE_MULTI_HOVER_SWEEP_PASS_NOT_EXECUTABLE",
                source_goals=list(SOURCE_GOALS), source_positions=list(SOURCE_POSITIONS),
                model_sha256=hashlib.sha256(path.read_bytes()).hexdigest(), legs=rows,
                maximum_writes=5, one_use_per_boot=True, export_each_leg_required=True,
                hardware_access=False, motion_authorized=False,
                physical_clearance_verified=False, physical_accuracy_verified=False,
                caveat="Unregistered meshless model omits cables, tool, and board; offline screen only.")
