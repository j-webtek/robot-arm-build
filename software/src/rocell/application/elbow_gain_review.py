"""Validate fixed gain evidence; no device I/O or tuning authority."""
...
REGISTERS = ((21,1),(22,1),(23,1))


def assess_gain(document, *, expected_boot, expected_capture):
    ...
    if (type(document) is not dict or set(document)!=fields or
            document['schema']!='rocell.elbow_gain.v1' or
            document['boot_id']!=expected_boot or document['capture_id']!=expected_capture or
            document['profile_id']!='roarm-m3-gain-reference-e8d5fc95a60f' or
            document['byte_order']!='little' or type(document['complete']) is not bool or
            type(document['reason']) is not str or
            type(document['reads']) is not list or len(document['reads'])>3):
        raise ValueError('Invalid gain identity or shape')
    ...
    for i,row in enumerate(document['reads']):
        ...
        if success:
            ...
            valid=False
        previous=end
    if document['complete'] and (not valid or len(values)!=3 or document['reason']!='GAIN_CAPTURED'):
        raise ValueError('Incomplete evidence claimed complete')
    complete=document['complete'] and valid and len(values)==3
    return dict(schema='rocell.elbow_gain_assessment.v1',
        category='CONTROLLER_REPORTED_GAINS' if complete else 'INCONCLUSIVE',
        register_values_raw=values if complete else {},
        gains_raw={name: values[str(address)] for name, address in
                   (("P", 21), ("D", 22), ("I", 23))} if complete else {},
        model_compatibility_verified=False, units_calibrated=False,
        motion_authorized=False, gain_changes_authorized=False)


def export_gain(root,document,*,expected_boot,expected_capture):
    subject=dict(expected_boot=expected_boot,expected_capture=expected_capture)
    assessment=assess_gain(document,**subject)
    exporter=WizardDiagnosticExporter(Path(root));exporter.prepare(create=True)
    saved=exporter.export({'mode':'gain-evidence'},[],attachments={
        'gain.json':canonical(document),'gain-subject.json':canonical(subject),
        'gain-assessment.json':canonical(assessment)})
    if replay_gain(root,Path(saved['path']).name)!=assessment:
        raise ValueError('Gain export replay differs')
    return dict(export_path=saved['path'],assessment=assessment)


def replay_gain(root,export_id):
    document,_=_read(Path(root),export_id,'attachment-gain.json')
    subject,_=_read(Path(root),export_id,'attachment-gain-subject.json')
    assessment,_=_read(Path(root),export_id,'attachment-gain-assessment.json')
    actual=assess_gain(document,**subject)
    if canonical(actual)!=canonical(assessment):raise ValueError('Gain assessment changed')
    return actual
