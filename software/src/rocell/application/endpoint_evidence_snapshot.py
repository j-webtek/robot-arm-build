...
SCHEMA = 'rocell.endpoint_evidence_snapshot.v1'
FILENAME = 'endpoint-evidence.json'
MAX_BYTES = 2*1024*1024


class FrozenEndpointReferences:
...
def decode_snapshot(raw,request):
    if type(request) is not EndpointTrialRequest or type(raw) is not bytes or not 0<len(raw)<=MAX_BYTES:
        raise ValueError('Bounded snapshot and exact endpoint request required')
    ...
    if (type(value) is not dict or set(value)!=expected or _canonical(value)!=raw
        ...
        raise ValueError('Frozen endpoint snapshot context mismatch')
...
def prepare_snapshot(workspace,request,*,reference_root,directory):
    ...
    reader = EndpointReferenceReader(request,workspace=workspace,reference_root=reference_root)
    reader()
