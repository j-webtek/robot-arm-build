"""Pure design for a nearby held-out shoulder-pair interpolation test."""
...
def draft(frozen, bounds, *, evidence_export, evidence_model_sha256,
          expected_lower_position=(2385, 1731), campaign_anchor=(2391, 1724)):
    """Create matched candidate/control campaigns; never infer current live state."""
    ...
    expected_lower_position = pair(expected_lower_position)
    proposal = propose(
        frozen, current_goals=LOWER, current_positions=expected_lower_position,
        desired=DESIRED, bounds=bounds[1:3], tested_primary_range=(2377, 2389),
        campaign_anchor=campaign_anchor)
    if (proposal["proposed_goals"] != [2388, 1726] or
            proposal["uncompensated_coupled_goals"] != UPPER or
            proposal["predictions"]["stateful_band"] != DESIRED):
        raise ValueError("Frozen held-out proposal differs")
    # Both terminal trials begin after the same lower target. An initial upper
    # leg would be a near-zero command after the candidate predecessor and is
    # correctly rejected by the native clear-response policy.
    candidate = [LOWER, proposal["proposed_goals"]]
    control = [LOWER, proposal["uncompensated_coupled_goals"]]
    return dict(
        schema="rocell.heldout_pair_validation.v1",
        source_bidirectional_evidence_export=evidence_export,
        frozen_model_sha256=frozen["sha256"], model_refitted=False,
        desired=DESIRED, expected_trial_start=list(expected_lower_position),
        matched_start_tolerance_counts=1,
        candidate=dict(manifest=_manifest(candidate, bounds),
                       roles=["conditioning_lower", "heldout_candidate"],
                       frozen_prediction=proposal["predictions"]["stateful_band"]),
        control=dict(manifest=_manifest(control, bounds),
                     roles=["conditioning_lower", "heldout_control"],
                     frozen_prediction=[2391, 1724]),
