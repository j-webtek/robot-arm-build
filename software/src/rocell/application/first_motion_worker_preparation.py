"""Coordinator preparation of one fixed commissioning worker; never launches it.
...
from .first_motion_evidence_snapshot import prepare_snapshot
from .first_motion_worker_claim import reserve_first_motion_launch
...
from rocell.providers.windows import first_motion_native_registration as protocol
...
class PreparedFirstMotionWorker:
    registration: WorkerProcessRegistration
    request: OwnedWorkerRequest


def prepare_first_motion_worker(workspace,request,*,review_root,session_id,measurement_operation_id,clock_ns=time.monotonic_ns):
    ...
    review_path = contained_path(root,body['attempt_id']+'-first-motion-reviews.json',label='signed bench reviews')
    def authenticate():
        ...
        return raw
    load_measurement_for_request(request,root=root,session_id=session_id,
        operation_id=measurement_operation_id,check_current=lambda:None,clock_ns=clock_ns)
    ...
    directory = contained_path(root,body['attempt_id']+'-first-motion-native-child',label='endpoint worker directory')
    ...
    launch = reserve_first_motion_launch(root,request,runtime_original=_canonical(runtime),
        review_bundle_sha256=hashlib.sha256(reviews).hexdigest(),now_ns=budget())
    payload = {'schema':PAYLOAD_SCHEMA,'root':str(root),'session_id':session_id,
    ...
    budget()
    return PreparedFirstMotionWorker(reg,outer)
