...
import re
from .compensated_shoulder_authorization import propose_compensated_step
from .wizard_diagnostic_coordinator import decode_diagnostic_json


class CompensatedStepReview:
    ...
    def authorize(self,now):
        ...
        self.plan=propose_compensated_step(self.references,boot=self.boot,command=self.command,now_us=now)
        return self.plan
    ...
    def accept(self,raw):
        ...
        if (type(doc) is not dict or n>=64 or doc.get('schema')!='rocell.shoulder_hold_event.v1'
                or doc.get('motion_contract')!='rocell.compensated_shoulder_step.v1'
        ...
        if n<3:
            ...
            if doc.get('requested_targets')!=p['command_goals'] or type(doc.get('speed')) is not int or doc['speed']!=20 or type(doc.get('acceleration')) is not int or doc['acceleration']!=1:
                raise ValueError('Local command parameters differ')
            if any(doc.get(field)!=p[field] for field in ('model_id','desired_positions','command_goals','predicted_positions','arrival_tolerance_counts')):
                raise ValueError('Compensation fields differ from reviewed plan')
            expected_errors=[positions[i+1]-p['desired_positions'][i] for i in range(2)]
            expected_residuals=[positions[i+1]-goals[i+1] for i in range(2)]
            for field,expected_values in (('desired_error_counts',expected_errors),('goal_residual_counts',expected_residuals)):
                values=doc.get(field)
                if type(values) is not list or len(values)!=2 or any(type(v) is not int for v in values) or values!=expected_values:
                    raise ValueError('Reported endpoint error differs from raw positions')
            for i,(position,goal) in enumerate(zip(positions,goals)):
                selected=i in (1,2);expected_goal=p['command_goals'][i-1] if selected and after else p['goals'][i]
                low=p['desired_positions'][0] if after and i==1 else p['positions'][i]
                high=p['desired_positions'][1] if after and i==2 else p['positions'][i]
            ...
            if after:
                ...
                arrived=not any(moving) and all(abs(positions[i]-p['desired_positions'][i-1])<=2 for i in (1,2))
