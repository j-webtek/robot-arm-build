"""Stage immutable absolute worker/original pins without launching a process."""
...
from .absolute_wrist_reference_reader import AbsoluteWristReferenceReader
from .absolute_wrist_worker_claim import reserve_absolute_wrist_launch
...
from rocell.providers.windows.owned_worker_process import OwnedWorkerRequest
...
class StagedAbsoluteWristRuntime:
...
@dataclass(frozen=True)
class PreparedAbsoluteWristWorker:
    registration: WorkerProcessRegistration
    request: OwnedWorkerRequest


def stage_absolute_wrist_runtime(workspace, *, root, attempt_id, controller_original, protocol_original):
    if type(attempt_id) is not str or not re.fullmatch('operation-[a-f0-9]{32}', attempt_id):
        raise ValueError('Host-selected absolute diagnostic attempt required')
    for raw in (controller_original, protocol_original):
        if type(raw) is not bytes:
            raise ValueError('Immutable structured originals required')
        value = decode_diagnostic_json(raw, maximum=128*1024)
        if type(value) is not dict or not value or canonical(value) != raw:
            raise ValueError('Canonical controller/protocol originals required')
    root = safe_root(Path(root))
    source = source_fingerprint(Path(workspace))
    directory = contained_path(root, attempt_id+'-absolute-wrist-native-child', label='absolute worker directory')
    ...
    if source_fingerprint(Path(workspace)) != source:
        raise ValueError('Source changed during absolute staging; originals retained without approval')
    return StagedAbsoluteWristRuntime(registration, source, attempt_id, root)


def prepare_absolute_wrist_worker(workspace, staged, request, *, review_original, clock_ns=time.monotonic_ns):
    ...
    def now():
        ...
        return tick
    now()
    refs = AbsoluteWristReferenceReader(request, workspace=workspace, root=staged.root)
    current = dict(refs())
    ...
    if (binding.origin is not EvidenceOrigin.PHYSICAL_OBSERVATION
        ...
        raise ValueError('Reviewed controller does not match absolute_wrist unit')
    ...
    publish_reservation_bytes(staged.root, body['attempt_id']+'-absolute-wrist-reviews.json',
                              review_original, maximum_bytes=8192)
    launch = reserve_absolute_wrist_launch(staged.root, request, runtime_original=runtime,
        review_bundle_sha256=hashlib.sha256(review_original).hexdigest(), now_ns=now())
    payload = dict(schema=protocol.PAYLOAD_SCHEMA, root=str(staged.root), absolute_wrist_intent=body,
    ...
    now()
    return PreparedAbsoluteWristWorker(staged.registration, outer)
