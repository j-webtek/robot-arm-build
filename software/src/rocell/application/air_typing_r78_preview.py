"""Offline count interpolation from the measured r77 post-fault pose."""
import hashlib
import math
...
from .air_typing_r78_campaign import SOURCE_GOALS,SOURCE_POSITIONS,TARGETS
...
def preview(model_path:Path)->dict:
    ...
    for ordinal,target in enumerate(TARGETS,1):
        maximum=max(abs(a-b) for a,b in zip(previous_goals,target))
        if maximum>80:raise ValueError("Unreviewed count step")
        ...
        if minimum_proxy<30:raise ValueError("Modeled self-separation below screen")
        rows.append(dict(leg=ordinal,original_finale_leg=ordinal+3,goals=list(target),
                         maximum_goal_step_counts=maximum,
    ...
    return dict(status="OFFLINE_R78_SWEEP_PASS_NOT_EXECUTABLE",
