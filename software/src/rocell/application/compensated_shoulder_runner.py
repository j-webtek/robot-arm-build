"""Finite host workflow. Offline compensated session; live release remains closed.
...
from .compensated_shoulder_authorization import sign_compensated_step
from .compensated_shoulder_review import CompensatedStepReview
...
def run_compensated_step(root,*,boot,key,exchange,authorized=False,origin='SIMULATION',
                   monotonic=time.monotonic,pause=time.sleep):
    from .shoulder_session_http import ShoulderSessionHTTP
    if authorized is not True or origin!='SIMULATION' or isinstance(exchange,ShoulderSessionHTTP):
        raise ValueError('Compensated step live release binding is not available')
    _hex(boot,16);_key(key);root=Path(root).resolve();command='compensated-step-1'
    exporter=WizardDiagnosticExporter(root);exporter.prepare(create=True)
    publish_reservation_bytes(root,'compensated-step-'+boot+'.json',canonical(dict(boot=boot,command=command)),maximum_bytes=2048)
    report=dict(schema='rocell.compensated_step_run.v1',boot_id=boot,command_id=command,origin=origin,
                state='IN_PROGRESS',operations=[],records=[],raw_exports=[])
    review=CompensatedStepReview(boot,command);deadline=monotonic()+40

    def save():
        result=exporter.export({'mode':'compensated-step-run'},[],attachments={'compensated-step-run.json':canonical(report)})
    ...
    def call(method,suffix,body=b'',*,settling=False):
        ...
        path=('/rocell/shoulder-settling/' if settling else '/rocell/compensated-step/')+suffix
    ...
    def retain(raw):
        saved=exporter.export({'mode':'compensated-step-raw'},[],attachments={'compensated-step-record.txt':raw})
        path=Path(saved['path'])
        if not verify_export(path)['valid'] or (path/'attachment-compensated-step-record.txt').read_bytes()!=raw:
    ...
    try:
        ...
        for _ in range(160):
            ...
            if (status.get('schema')!='rocell.compensated_step_status.v1' or status.get('boot_id')!=boot
            ...
            if state=='AUTHORIZATION':
                ...
                token=sign_compensated_step(challenge,plan,key)
                ...
                report.update(state='COMPENSATED_STEP_OBSERVED',final_positions=review.last_positions)
