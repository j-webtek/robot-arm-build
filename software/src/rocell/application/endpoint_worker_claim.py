"""One-use launch/child records for future supervised endpoint execution.
...
def _path(root, request, stage):
    if type(request) is not EndpointTrialRequest or stage not in {'launch','claimed'}:
        raise ValueError('Exact endpoint request and worker stage required')
    return contained_path(safe_root(Path(root)), request.to_dict()['attempt_id']+
                          '-endpoint-worker-'+stage+'.json', label='endpoint worker record')
...
def reserve_endpoint_launch(root, request, *, runtime_original, review_bundle_sha256, now_ns):
    ...
    body = {'schema':'rocell.endpoint_worker_launch.v1','request':request.to_dict(),
...
class EndpointWorkerClaim:
    ...
    def consume(self,request,*,current_source_sha256,current_runtime_sha256,now_ns):
        with self._lock:
            ...
            self._used = True
            if (os.getpid()!=self._pid or type(request) is not EndpointTrialRequest
...
def _verify_launch(root,request,launch_sha,source_sha,runtime_sha,now_ns):
    if type(request) is not EndpointTrialRequest: raise ValueError('Exact request required')
    request.require_start_time(now_ns)
    ...
    if (set(body)!=expected or body['schema']!='rocell.endpoint_worker_launch.v1'
    ...
    review_path = contained_path(safe_root(Path(root)),request.to_dict()['attempt_id']+'-bench-reviews.json',label='bench review original')
...
def claim_endpoint_worker(root,request,*,launch_sha256,current_source_sha256,current_runtime_sha256,now_ns):
    _verify_launch(root,request,launch_sha256,current_source_sha256,current_runtime_sha256,now_ns)
    body = {'schema':'rocell.endpoint_worker_claim.v1','request_sha256':request.request_sha256,
    ...
    return EndpointWorkerClaim(_ISSUER,request,safe_root(Path(root)),launch_sha256,raw)


def verify_endpoint_worker_receipt(root, request, *, claim_sha256, launch_sha256,
    ...
    if (set(body)!={'schema','request_sha256','launch_sha256','pid','claimed_ns',
            ...
            or body['schema']!='rocell.endpoint_worker_claim.v1'
