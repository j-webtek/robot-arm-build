...
from .p4_repeat_campaign_review import GOALS, POSITIONS
from .p4_correction_comparison import ORDER, COMMANDS
...
def assess_leg(raw, *, boot, leg, previous=None):
    ...
    if type(raw) is not bytes or len(raw)!=1130 or raw[:10]!=b'RCWRCMP001' or raw[26]!=leg:
...
class P4CorrectionHost:
    ...
    def run_once(self, *, clock=time.monotonic, pause=time.sleep):
        ...
        def call(method, suffix, body=b''):
            return self.transport(method, '/rocell/p4-correction/'+suffix, body)
        try:
            if call('POST','start',b'P4C16')!=b'CAPTURING_START':
            ...
            for leg in range(1,17):
                ...
                saved=exporter.export({'mode':'p4-correction-leg'},[],attachments={
                    'p4-correction-record.hex.txt':raw.hex().encode(), 'p4-correction-assessment.json':canonical(row)})
            ...
            saved=exporter.export({'mode':'p4-correction-fault'},[],attachments={
                'p4-correction-fault.json':canonical(dict(boot=self.boot,leg=leg,
                ...
                'p4-correction-fault-record.hex.txt':raw.hex().encode()})
            if not verify_export(Path(saved['path']))['valid']:
