"""Seven-leg B-A finale: independent raw verifier and export-gated host."""
...
from .air_typing_r77_preview import SOURCE_GOALS, SOURCE_POSITIONS
...
PHASES=("B_virtual_downstroke","B_retract","BA_mid","A_return_travel",
        "A_return_hover","A_virtual_downstroke","A_retract")


def assess_leg(raw:bytes,*,boot:str,leg:int,previous=None)->dict:
    if type(leg) is not int or not 1<=leg<=7 or type(raw) is not bytes or len(raw)!=1130:
        raise ValueError("Invalid finale record")
    if raw[:10]!=b"RCAIRAB301" or raw[26]!=leg:
        raise ValueError("Wrong finale domain or ordinal")
    ...
    if leg==1:
        ...
        positions,goals,tolerance=SOURCE_POSITIONS,SOURCE_GOALS,3
    else:
        if previous is None or previous.get("boot")!=boot or previous.get("leg")!=leg-1:
            raise ValueError("Missing verified previous leg")
        positions,goals,tolerance=previous["final_positions"],previous["final_goals"],1
    ...
    for i,joint in enumerate(initial):
        if joint["goal"]!=goals[i] or abs(joint["position"]-positions[i])>tolerance:
    ...
    return dict(status="FINALE_LEG_ENDPOINT_VERIFIED",boot=boot,leg=leg,
...
class AirTypingFinaleHost:
    ...
    def run_once(self,*,clock=time.monotonic,pause=time.sleep):
        if self.used:
            raise ValueError("One-use finale host consumed")
        self.used=True
        ...
        def call(method,suffix,body=b""):
            return self.transport(method,"/rocell/air-type-final/"+suffix,body)
        try:
            if call("POST","start",b"AIR7")!=b"CAPTURING_START":
            ...
            for leg in range(1,8):
                ...
                while True:
                    ...
                    if status not in [f"{state}|{leg}".encode() for state in
                        ...
                        raise ValueError("Unexpected finale status")
                    pause(0.1)
                ...
                saved=exporter.export({"mode":"r77-air-typing-leg"},[],attachments={
                    "air-typing-final-record.hex.txt":raw.hex().encode(),
                    "air-typing-final-assessment.json":canonical(row)})
                if not verify_export(Path(saved["path"]))["valid"]:
                    raise ValueError("Leg export invalid")
                exports.append(saved["path"])
                if call("POST","receipt",f'{leg}:{row["record_sha256"]}'.encode())!=(
                        b"COMPLETE" if leg==7 else f"READY|{leg+1}".encode()):
                    raise ValueError("Receipt uncertain; no retry")
                rows.append(row)
                if leg<7 and call("POST","next",str(leg+1).encode())!=b"CAPTURING_START":
                    raise ValueError("Next-leg admission uncertain; no retry")
            return dict(status="FINALE_COMPLETE",rows=rows,exports=exports,
                        follow_on_movement_authorized=False)
        except Exception as error:
            saved=exporter.export({"mode":"r77-air-typing-fault"},[],attachments={
                "air-typing-final-fault.json":canonical(dict(boot=self.boot,leg=leg,
                    error_type=type(error).__name__,completed_legs=len(rows),
                    prior_exports=exports,source_kind=self.source_kind,
                    last_controller_status=last_status,retry_allowed=False)),
                "air-typing-final-fault-record.hex.txt":raw.hex().encode()})
            if not verify_export(Path(saved["path"]))["valid"]:
                raise ValueError("Fault export invalid") from error
            raise ValueError("Finale stopped; evidence: "+saved["path"]) from error
