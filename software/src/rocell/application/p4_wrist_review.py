"""Measured P4E to P4 wrist-only contract and sampled geometry review."""
import hashlib
...
SOURCE_POSITIONS = (2047,2225,1890,2716,1912,2041,2047)
SOURCE_GOALS = (2047,2217,1897,2711,1915,2040,2047)
TARGET_GOALS = (2047,2217,1897,2711,1980,2040,2047)


def review_p4_wrist(raw, *, expected_boot, model_path):
    assessment = assess_large_pose_relief_record(raw, expected_boot=expected_boot, profile='P4E')
    endpoint = decode_large_pose_relief_record(raw, profile='P4E')['endpoint'][-1]['joints']
    ...
    if positions != SOURCE_POSITIONS or goals != SOURCE_GOALS:
        raise ValueError('Record differs from pinned measured P4E endpoint')
    ...
    return dict(schema='rocell.p4_wrist_review.v1', source_assessment=assessment,
        ...
        absolute_target_minus_measured_position_counts=68,
        ...
        assumptions=['Single logical wrist joint; shoulder and elbow held fixed',
                     'Nominal endpoint assumes the measured -3-count wrist bias persists',
