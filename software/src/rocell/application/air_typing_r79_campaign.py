"""Independent six-leg controller-record verifier and export-gated host."""
...
from .air_typing_repeat_recipe import PHASES,SOURCE_GOALS,SOURCE_POSITIONS,TARGETS
...
def assess_leg(raw:bytes,*,boot:str,leg:int,previous=None)->dict:
    if type(leg) is not int or not 1<=leg<=6 or type(raw) is not bytes or len(raw)!=1130:
        raise ValueError("Invalid repeat record")
    if raw[:10]!=b"RCAIRAB501" or raw[26]!=leg:
        raise ValueError("Wrong repeat domain or ordinal")
    target=TARGETS[leg-1]
    if int.from_bytes(raw[27:29],"big")!=target[4]:
        raise ValueError("Wrong repeat target")
    ...
    return dict(status="A_REPEAT_LEG_ENDPOINT_VERIFIED",boot=boot,leg=leg,
...
class AirTypingRepeatHost:
    ...
    def run_once(self,*,clock=time.monotonic,pause=time.sleep):
        if self.used:raise ValueError("One-use repeat host consumed")
        self.used=True
        ...
        def call(method,suffix,body=b""):
            return self.transport(method,"/rocell/air-type-repeat/"+suffix,body)
        try:
            if call("POST","start",b"AIR6")!=b"CAPTURING_START":
            ...
            for leg in range(1,7):
                ...
                while True:
                    ...
                    if status not in [f"{state}|{leg}".encode() for state in
                        ...
                        raise ValueError("Unexpected repeat status")
                    pause(0.1)
                ...
                saved=exporter.export({"mode":"r79-air-typing-repeat-leg"},[],attachments={
                    "air-typing-repeat-record.hex.txt":raw.hex().encode(),
                    "air-typing-repeat-assessment.json":canonical(row)})
                if not verify_export(Path(saved["path"]))["valid"]:
                    raise ValueError("Leg export invalid")
                exports.append(saved["path"])
                if call("POST","receipt",f'{leg}:{row["record_sha256"]}'.encode())!=(
                        b"COMPLETE" if leg==6 else f"READY|{leg+1}".encode()):
                    raise ValueError("Receipt uncertain; no retry")
                rows.append(row)
                if leg<6 and call("POST","next",str(leg+1).encode())!=b"CAPTURING_START":
                    raise ValueError("Next-leg admission uncertain; no retry")
            return dict(status="A_REPEAT_COMPLETE",rows=rows,exports=exports,
                        follow_on_movement_authorized=False)
        except Exception as error:
            saved=exporter.export({"mode":"r79-air-typing-repeat-fault"},[],attachments={
                "air-typing-repeat-fault.json":canonical(dict(boot=self.boot,leg=leg,
                    ...
                    retry_allowed=False)),
                "air-typing-repeat-fault-record.hex.txt":raw.hex().encode()})
            if not verify_export(Path(saved["path"]))["valid"]:
                raise ValueError("Fault export invalid") from error
            raise ValueError("A-repeat campaign stopped; evidence: "+saved["path"]) from error
