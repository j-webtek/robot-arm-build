"""Retain first_motion IPC originals before interpretation; no launch or device APIs.
...
from .first_motion_native_protocol import decode_request
from .first_motion_native_result import decode_result, validate_result, MAX_RESULT_BYTES


def publish_supervised_first_motion_result(root, *, registration, request, result):
    ...
    from .first_motion_native_protocol import REQUEST_SCHEMA, RESULT_SCHEMA
    ...
    # first_motion observation. Those observations remain available in the summary.
    ...
    return publish_first_motion_result(root,request_raw=raw,stdout=result.stdout,stderr=result.stderr,
...
def publish_first_motion_result(root, *, request_raw, stdout, stderr, owned_process_id,
    ...
    prefix = wire['attempt_id']+'-first_motion-'
    originals = {}
    ...
    report = {'schema':'rocell.first_motion_retained_result.v1','request_sha256':request.request_sha256,
              'status':'RESULT_REJECTED','originals':originals,'summary':None,
              'returncode':returncode,'process_tree_closed':process_tree_closed,
              'claim_receipt_verified':False,'campaign_advance_allowed':False,
