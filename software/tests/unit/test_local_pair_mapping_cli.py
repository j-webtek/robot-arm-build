...
def cli(monkeypatch):
    ...
    spec=importlib.util.spec_from_file_location('mapping_batch_cli_test',
                                                scripts/'run_local_pair_mapping_batch.py')
    module=importlib.util.module_from_spec(spec);spec.loader.exec_module(module)
    return module


@pytest.mark.parametrize('fault',[None,'plan','reserved','pose_reserved'])
def test_preflight_binds_r48_plan_and_one_use_boot(cli,tmp_path,monkeypatch,fault):
    boot='ab'*16
    monkeypatch.setattr(cli,'review_recovery_startup',lambda *args,**kwargs:
                        {'expected_boot':boot,'address':'127.0.0.1'})
    saved=plan_mapping_batch()
    if fault=='plan':
        saved=dict(saved);saved['revision']=49
    monkeypatch.setattr(cli,'_read',lambda *args:(saved,'verified'))
    exports=tmp_path/'runs/wizard-exports';exports.mkdir(parents=True)
    if fault=='reserved':(exports/f'r48-capture-{boot}.json').write_text('{}')
    ...
    if fault:
        ...
        assert binding['mapping_plan']==plan_mapping_batch()
        assert root==exports and claim.name==f'r48-capture-{boot}.json'
