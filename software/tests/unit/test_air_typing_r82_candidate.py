"""Native and host checks for the nearby held-out elbow target candidate."""
...
from rocell.application.air_typing_elbow_shift_recipe import TARGETS
from rocell.application.air_typing_r82_campaign import AirTypingElbowShiftHost,assess_leg
from rocell.application.wizard_diagnostic_export import verify_export


ROOT=Path(__file__).resolve().parents[2]
STAGE=ROOT/".firmware-tools/configured-diagnostic-candidate-r82/RoArm-M3_example"
...
def native(tmp_path_factory):
    ...
    target=tmp_path_factory.mktemp("r82-native")/"owner.exe"
...
def test_exact_eight_records_and_export_gated_host(native,tmp_path):
    ...
    for leg,raw in enumerate(records,1):
        assert len(raw)==1130 and raw[:10]==b"RCAIRAB801" and raw[26]==leg
    ...
    class Transport:
        ...
        def __call__(self,method,path,body=b""):
            ...
            assert path.startswith("/rocell/air-elbow-shift/")
            if suffix=="start":
                assert method=="POST" and body==b"AIRH8"
    ...
    transport=Transport()
    host=AirTypingElbowShiftHost(transport,boot="ab"*16,export_root=tmp_path,
                                 source_kind="simulation")
    result=host.run_once()
    assert result["status"]=="ELBOW_SHIFT_COMPLETE"
...
def test_native_route_fixed_selector_and_read_only_fault(tmp_path):
    ...
    changes={
        ...
        '/rocell/air-type/':'/rocell/air-elbow-shift/',
        'AIR17':'AIRH8',
    }
    for old,new in changes.items():
  ...
  failed.register_routes();web2.body="AIRH8";web2.routes["/rocell/air-elbow-shift/start"]();
  assert(web2.status==202);web2.body="";
  for(unsigned i=0;i<3;++i){clock.now+=150000;failed.poll();}
  web2.routes["/rocell/air-elbow-shift/status"]();
  assert(web2.response=="SOURCE_POSE_REJECTED|1"&&rejected.writes==0);
  web2.routes["/rocell/air-elbow-shift/source-fault"]();
