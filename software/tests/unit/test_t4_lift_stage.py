...
def test_exact_specialization_and_tamper_rejection():
    spec = importlib.util.spec_from_file_location('t4_stage', ROOT/'scripts/stage_r68_t4_lift.py')
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    base = ROOT/'.firmware-tools/configured-diagnostic-candidate-r64/RoArm-M3_example'
    ...
    modified = dict(files)
    modified['large_pose_relief_owner.h'] = files['large_pose_relief_owner.h'].replace(b'RCP2LIFT01', b'WRONGTOKEN')
    with pytest.raises(ValueError, match='Expected unique reviewed source'):
