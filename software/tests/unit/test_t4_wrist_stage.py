...
def test_exact_specialization_and_tamper_rejection():
    spec = importlib.util.spec_from_file_location('t4_stage', ROOT/'scripts/stage_r69_t4_wrist.py')
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    base = ROOT/'.firmware-tools/configured-diagnostic-candidate-r67/RoArm-M3_example'
    ...
    modified = dict(files)
    modified['large_pose_relief_owner.h'] = files['large_pose_relief_owner.h'].replace(b'RCP3WRST01', b'WRONGTOKEN')
