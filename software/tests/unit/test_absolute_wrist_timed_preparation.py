...
from rocell.application.absolute_wrist_worker_preparation import stage_absolute_wrist_runtime, prepare_absolute_wrist_worker
...
def fixture(tmp_path, monkeypatch):
    ...
    authority = BenchReviewAuthority(b'Q'*32).for_absolute_wrist_diagnostic()
    monkeypatch.setattr(bench_review_key, 'load_host_absolute_wrist_review_authority', lambda _: authority)
...
def test_stage_then_finalize_without_device_or_process(tmp_path, monkeypatch):
    ...
    with pytest.raises(FileExistsError):
        stage_absolute_wrist_runtime(workspace, root=tmp_path, attempt_id=request.to_dict()['attempt_id'],
            controller_original=controller, protocol_original=protocol)
    ...
    with pytest.raises(Exception):
        prepare_absolute_wrist_worker(workspace, staged, request, review_original=signed, clock_ns=lambda: 2_000_000_000)
...
def test_finalize_refuses_changed_or_missing_evidence(tmp_path, monkeypatch, fault):
    ...
    if fault == 'key':
        ...
        monkeypatch.setattr(bench_review_key, 'load_host_absolute_wrist_review_authority', missing)
    with pytest.raises(ValueError):
        prepare_absolute_wrist_worker(workspace, staged, request, review_original=signed, clock_ns=lambda: now)
    assert not (tmp_path/(request.to_dict()['attempt_id']+'-absolute-wrist-launch.json')).exists()
