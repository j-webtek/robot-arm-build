...
def runner(monkeypatch):
    ...
    spec = importlib.util.spec_from_file_location('tested_p2_runner', ROOT/'scripts/run_p4_repeat_campaign.py')
...
def test_live_runner_binds_profile_and_claim(runner, monkeypatch, tmp_path, wrong_boot):
    ...
    class Host:
        def __init__(self, client, **kwargs):
            assert claim.exists()
            assert kwargs['source_kind'] == 'controller_feedback' and kwargs['boot'] == boot
        def run_once(self):
            calls.append('write')
            raise TimeoutError('uncertain')
    monkeypatch.setattr(runner, 'P4RepeatHost', Host)
...
def test_real_source_replay_and_claim_rejection(runner, monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(runner, 'review_recovery_startup', lambda root, ident, revision: (
        {'expected_boot':'ab'*16} if revision == 72 else pytest.fail('Wrong firmware binding')))
    exports = ROOT/'runs/wizard-exports'
    source = exports/runner.SOURCE
    assert source.is_dir(), 'Pinned P4E source export required for integration review'
