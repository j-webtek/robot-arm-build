"""Native and host verification for staged five-leg multi-joint hover."""
...
from rocell.application.air_typing_multi_hover_recipe import TARGETS
from rocell.application.air_typing_r84_campaign import AirTypingMultiHoverHost, assess_leg
from rocell.application.wizard_diagnostic_export import verify_export


ROOT = Path(__file__).resolve().parents[2]
STAGE = ROOT / ".firmware-tools/configured-diagnostic-candidate-r84/RoArm-M3_example"
...
def specialized_owner():
    ...
    changes = {
        ...
        '{2047,2225,1890,2716,1979,2041,2047}':'{2041,2082,2033,2622,2233,2041,2047}',
        '{2047,2217,1897,2711,1980,2040,2047}':'{2047,2075,2039,2620,2233,2040,2047}',
...
def native(tmp_path_factory):
    ...
    target = tmp_path_factory.mktemp("r84-native") / "owner.exe"
...
def test_five_native_records_and_export_gated_host(native, tmp_path):
    ...
    for leg, raw in enumerate(records, 1):
        assert len(raw) == 1130 and raw[:10] == b"RCAIRABA01" and raw[26] == leg
        prior = assess_leg(raw, boot="ab"*16, leg=leg, previous=prior)
        assert prior["target_goals"] == list(TARGETS[leg-1])
        assert prior["selected_joints"] == ([3] if leg == 1 else [0,1,2,3,4])
    ...
    class Transport:
        ...
        def __call__(self, method, path, body=b""):
            ...
            assert path.startswith("/rocell/air-multi-hover/")
            if suffix == "start":
                assert method == "POST" and body == b"AIRM5"
                return b"CAPTURING_START"
            if suffix == "status":
                return f"AWAITING_EXPORT|{self.leg}".encode()
            if suffix == "record":
                return records[self.leg-1].hex().encode()
            if suffix == "receipt":
                assert body == f'{self.leg}:{hashlib.sha256(records[self.leg-1]).hexdigest()}'.encode()
                self.receipts.append(self.leg)
                self.leg += 1
                return b"COMPLETE" if self.leg == 6 else f"READY|{self.leg}".encode()
            if suffix == "next":
                assert body == str(self.leg).encode()
                return b"CAPTURING_START"
            raise AssertionError(path)
    transport = Transport()
    host = AirTypingMultiHoverHost(transport,boot="ab"*16,export_root=tmp_path,
                                   source_kind="simulation")
    result = host.run_once()
    assert result["status"] == "MULTI_HOVER_COMPLETE"
    assert transport.receipts == list(range(1,6))
...
def test_native_route_requires_fixed_selector_and_exposes_read_only_source_fault(tmp_path):
    compiler = shutil.which("clang++")
    if not compiler:
        pytest.skip("Native compiler unavailable")
    ...
    changes = {
        ...
        '/rocell/air-type/':'/rocell/air-multi-hover/',
        'AIR17':'AIRM5',
    }
    for old,new in changes.items():
  ...
  failed.register_routes();web2.body="AIRM5";web2.routes["/rocell/air-multi-hover/start"]();
  assert(web2.status==202);web2.body="";
  for(unsigned i=0;i<3;++i){clock.now+=150000;failed.poll();}
  web2.routes["/rocell/air-multi-hover/status"]();
  assert(web2.response=="SOURCE_POSE_REJECTED|1"&&rejected.writes==0);
  web2.routes["/rocell/air-multi-hover/source-fault"]();
