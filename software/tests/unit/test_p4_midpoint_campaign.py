...
from rocell.application.p4_midpoint_campaign import assess_leg, P4MidpointHost
...
def native(tmp_path_factory):
    ...
    result=subprocess.run([compiler,'-std=c++17','-I'+str(ROOT/'firmware/diagnostics'),
        ...
        str(ROOT/'firmware/diagnostics/test_p4_midpoint_owner.cpp'),'-o',str(target)],capture_output=True,text=True)
...
def test_native_records_independently_verified(records):
    ...
    for leg,raw in enumerate(records,1):
        ...
        assert previous['endpoint_error_counts']==(-2 if leg % 2 == 0 else 2)
...
def test_simulated_correction_scoring(records):
    from rocell.application.p4_midpoint_scoring import score_records
    result=score_records(records,boot='ab'*16)
    assert result['result']=='LOCAL_SCREEN_PASS'
    assert result['metrics']['baseline']['errors_counts']==[-1]*4
    assert result['metrics']['candidate']['errors_counts']==[0]*4
    assert result['mae_improvement_counts']==1
...
class Transport:
    ...
    def __call__(self,method,path,body=b''):
        ...
        if suffix=='start': assert body==b'P4M16';return b'CAPTURING_START'
...
def test_full_host_campaign(records,tmp_path):
    transport=Transport(records);host=P4MidpointHost(transport,boot='ab'*16,export_root=tmp_path)
...
def test_lost_receipt_stops_without_retry(records,tmp_path,leg):
    ...
    host=P4MidpointHost(transport,boot='ab'*16,export_root=tmp_path)
...
def test_export_failure_never_receipts(records,tmp_path,monkeypatch,leg):
    from rocell.application import p4_midpoint_campaign as module
    ...
    with pytest.raises(ValueError,match='Campaign stopped'):
        P4MidpointHost(transport,boot='ab'*16,export_root=tmp_path).run_once()
