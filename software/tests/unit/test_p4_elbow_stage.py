...
def test_exact_specialization_and_tamper_rejection():
    spec = importlib.util.spec_from_file_location('t4_stage', ROOT/'scripts/stage_r70_p4_elbow.py')
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    base = ROOT/'.firmware-tools/configured-diagnostic-candidate-r69/RoArm-M3_example'
    ...
    modified = dict(files)
    modified['large_pose_relief_owner.h'] = files['large_pose_relief_owner.h'].replace(b'RCT4WRST01', b'WRONGTOKEN')
