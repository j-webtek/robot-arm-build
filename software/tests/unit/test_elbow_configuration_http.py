...
import pytest
from rocell.application import elbow_configuration_capture as capture


@pytest.mark.parametrize('method',['POST','GET'])
@pytest.mark.parametrize('fault',[None,'truncated','status','oversize','duplicate','encoded','slow'])
def test_bounded_configuration_exchange(monkeypatch,method,fault):
    ...
    if fault:
        with pytest.raises((ValueError,OSError)):capture.configuration_exchange('192.168.0.225',method)
    else:assert capture.configuration_exchange('192.168.0.225',method)==b'{}'
    assert len(created)==sock.connects==len(sock.sends)==1 and sock.closed
    path='capture' if method=='POST' else 'result'
    assert sock.sends[0].startswith(f'{method} /rocell/elbow-configuration/{path} HTTP/1.1\r\n'.encode())
...
def test_invalid_operation_never_creates_socket(monkeypatch):
    ...
    for method in ('PUT','DELETE','POST /js'):
        with pytest.raises(ValueError):capture.configuration_exchange('192.168.0.225',method)
