...
SOURCE=ROOT/'runs/wizard-exports/wizard-20260923T220934352526Z-00cc861d428f4b168b5c1ead42c5de8e'
...
def test_retained_endpoint_review():
    ...
    review=review_p3_wrist(raw,expected_boot='e61c5767d2d5b0616ac34cd847b2f59d',model_path=MODEL)
    assert review['selected_servo_ids']==[15]
    assert review['command_delta_counts']==65
    assert review['absolute_target_minus_measured_position_counts']==66
    ...
    with pytest.raises(ValueError,match='identity mismatch'):
        review_p3_wrist(raw,expected_boot='ab'*16,model_path=MODEL)
    with pytest.raises(ValueError,match='framing'):
        review_p3_wrist(raw[:-1],expected_boot='ab'*16,model_path=MODEL)
...
def test_native_wrist_policy(tmp_path):
    ...
    result=subprocess.run([compiler,'-std=c++17',str(ROOT/'firmware/diagnostics/test_p3_wrist_policy.cpp'),'-o',str(target)],capture_output=True,text=True,timeout=30)
