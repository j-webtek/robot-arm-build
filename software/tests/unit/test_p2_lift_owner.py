...
def executable(tmp_path_factory):
    ...
    candidate = ROOT/'.firmware-tools/configured-diagnostic-candidate-r64/RoArm-M3_example'
    if not candidate.is_dir():
        pytest.fail('Stage the pinned r64 candidate before its native tests')
    target = tmp_path_factory.mktemp('p2-native')/'owner.exe'
    result = subprocess.run([compiler, '-std=c++17', '-I'+str(candidate),
        str(ROOT/'firmware/diagnostics/test_p2_lift_owner.cpp'), '-o', str(target)],
...
def test_exact_candidate(executable, mode):
    ...
    if mode == 'success':
        ...
        assessment = assess_large_pose_relief_record(raw, expected_boot='ab'*16, profile='P2L')
        assert assessment['status'] == 'P2L_JOINT_ENDPOINT_MEASURED'
        assert assessment['position_delta_counts'] == [0,-65,65,0,0,0,0]
        assert assessment['synchronized_servo_ids'] == [12,13]
...
def test_host_export_and_receipt(executable, tmp_path, wrong_domain):
    ...
    def transport(method, path, body=b''):
        ...
        return {'start':b'CAPTURING_START', 'status':b'AWAITING_DURABLE_EXPORT|1',
                'record':raw.hex().encode(), 'receipt':b'LARGE_POSE_P2L_RECORDED'}[path.rsplit('/',1)[1]]
    host = LargePoseReliefHost(transport, export_root=tmp_path, boot='ab'*16, profile='P2L')
    if wrong_domain:
        with pytest.raises(ValueError, match='framing'):
        ...
        assert result['assessment']['target_pose'] == 'P2L'
        assert calls[-1][1].endswith('receipt')
    assert calls[0][2] == b'P2L'
