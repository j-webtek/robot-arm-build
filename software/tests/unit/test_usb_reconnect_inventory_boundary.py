"""Real v12 facts and Collect transfer with explicitly modeled persistence.
...
def test_exact_original_inventory_facts_and_complete_collect_transfer(
    ...
    facts = owner._facts_provider(workflow, ready[1], campaign, guard)
    ...
    extra = reference(
        canonical({"MODELED": "unreviewed extra"}), "unreviewed-reconnect-extra"
    )
    changed_cases = (
    ...
    for evidence in changed_cases:
        ...
        assert raised.value.code == "USB_RECONNECT_CURRENT_INVENTORY_CHANGED"
    ...
    reconnect_query(made, stop="campaign")
    ...
    class ModelDispatch:
        ...
        def perform(self, *, request_key, cancellation):
            assert (
                request_key
                == "usb-reconnect-"
                + workflow["usb_qualification_reconnect"]["phase_id"]
            )
    ...
    monkeypatch.setattr(reconnect, "M1PhysicalUsbIdentityPersistence", ModelPersistence)
    monkeypatch.setattr(reconnect, "PhysicalUsbIdentityDispatchOwner", ModelDispatch)
    phase = owner._reconnect
    phase.attempt = dict(
        action_id=reconnect.COLLECT,
        phase_id=workflow["usb_qualification_reconnect"]["phase_id"],
        records={},
    )
    ...
    row = final["usb_qualification_reconnect"]
