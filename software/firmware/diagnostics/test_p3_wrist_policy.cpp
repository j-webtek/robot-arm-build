...
static ShoulderPreloadPose pose(int n,bool after){
  ...
  for(int i=0;i<7;++i){
    p.goal[i]=after?P3WristPolicy::target_goals[i]:P3WristPolicy::source_goals[i];
    p.position[i]=P3WristPolicy::source_positions[i];p.torque[i]=1;
  }
  if(after)p.position[4]=1849;
...
int main(){
  ...
  assert(P3WristPolicy::source(before,1400000));
  assert(P3WristPolicy::endpoint(before,after,1500000,2400000));
  auto bad=after[2];position(bad,1,bad.position[1]+3);
  assert(!P3WristPolicy::bounded_endpoint_sample(before[2],bad));
  bad=after[2];position(bad,4,1782);
  assert(!P3WristPolicy::bounded_endpoint_sample(before[2],bad));
  bad=after[2];position(bad,4,1865);
  assert(!P3WristPolicy::bounded_endpoint_sample(before[2],bad));
  bad=after[2];bad.goal[4]++;
  assert(!P3WristPolicy::bounded_endpoint_sample(before[2],bad));
  for(auto& p:after)position(p,4,1820);
  assert(!P3WristPolicy::endpoint(before,after,1500000,2400000));
  position(before[2],4,1788);
  assert(!P3WristPolicy::source(before,1400000));
}
