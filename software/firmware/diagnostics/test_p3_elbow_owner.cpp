...
static ShoulderPreloadPose pose(unsigned index){
  ...
  if(after){sample.position[3]=2779;}
...
int main(int argc,char** argv){
  ...
  auto acquire=[&](ShoulderPreloadPose& out){
    ...
    if(mode=="reverse"&&acquisition>=4)out.position[3]=2846;
  ...
  auto write=[&](uint8_t a,uint16_t x,
    ...
    if(a!=14||x!=2777||speed!=20||acc!=1)return false;
    ++transmissions;return mode!="write_uncertain";
  };
  auto evidence=[&](const char* event,const ShoulderPreloadPose&){
    return !(mode=="evidence_failure"&&std::strcmp(event,"LARGE_POSE_P3E_INTENT")==0);};
