// Candidate read-only routes. No actuator commands or arbitrary register API.
#pragma once
#include "shoulder_configuration_json.h"
namespace rocell_diag {
template<class Bus,class Clock,class Web>
class ShoulderConfigurationRoutes {
 public:
  ShoulderConfigurationRoutes(Bus& bus,Clock& clock,Web& web,const char* boot,
  ...
  void register_routes(){
    ...
    registered_=true;
    web_.on("/rocell/shoulder-configuration/capture",HTTP_POST,[this](){
      ...
      retained_=shoulder_configuration_json(snapshot_,boot_,"shoulder-config-1",response_,sizeof(response_));
      if(!retained_){send(500,"{\"error\":\"SERIALIZATION_FAILED\"}");return;}
      send(200,response_);
    });
    web_.on("/rocell/shoulder-configuration/result",HTTP_GET,[this](){
      if(web_.args()!=0){send(400,"{\"error\":\"NO_PARAMETERS_ALLOWED\"}");return;}
 ...
 private:
  ...
  ShoulderConfigurationSnapshot snapshot_;char response_[8192]={};
