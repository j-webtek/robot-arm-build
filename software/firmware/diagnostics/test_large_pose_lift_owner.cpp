...
static ShoulderPreloadPose pose(unsigned index){
  ...
  for(int i=0;i<7;++i){
    sample.goal[i]=after?LargePoseLiftPolicy::target_goals[i]:LargePoseLiftPolicy::source_goals[i];
    sample.position[i]=LargePoseLiftPolicy::source_positions[i];sample.torque[i]=1;
  }
  if(after){sample.position[1]=2356;sample.position[2]=1759;sample.position[4]=1652;}
...
int main(int argc,char** argv){
  ...
  LargePoseLiftOwner owner;Clock clock;unsigned acquisition=0,transmissions=0;
  ...
  auto acquire=[&](ShoulderPreloadPose& out){
    ...
    if(mode=="source_rejected"&&acquisition==2)out.goal[4]++;
    if(mode=="prewrite_changed"&&acquisition==3)out.position[0]+=2;
    if(mode=="wrong_goal"&&acquisition==6)out.goal[4]++;
    if(mode=="passive_drift"&&acquisition==6)out.position[3]+=3;
    if(mode=="reverse"&&acquisition>=4)out.position[4]=1588;
  ...
  auto write=[&](uint8_t a,uint8_t b,uint8_t c,uint16_t x,uint16_t y,uint16_t z,
                 uint16_t speed,uint8_t acc){
    if(a!=12||b!=13||c!=15||x!=2348||y!=1766||z!=1654||speed!=20||acc!=1)return false;
    ++transmissions;return mode!="write_uncertain";
  };
  auto evidence=[&](const char* event,const ShoulderPreloadPose&){
    return !(mode=="evidence_failure"&&std::strcmp(event,"LARGE_POSE_T1_INTENT")==0);};
  auto admitted=[](){return true;};
  for(unsigned i=0;i<7&&owner.state()!=LargePoseLiftOwner::State::Fault&&
      owner.state()!=LargePoseLiftOwner::State::AwaitExport;++i){
    clock.now=pose(i).finished_us+1000;owner.poll(acquire,write,clock,evidence,admitted);
  }
  if(mode=="success"){
    uint8_t boot[16],record[1133];for(auto& byte:boot)byte=0xab;
    if(owner.state()!=LargePoseLiftOwner::State::AwaitExport||owner.writes()!=1||
  ...
  return owner.state()==LargePoseLiftOwner::State::Fault&&owner.writes()==expected&&
