...
#pragma once
#include "p4_correction_policy.h"
#include <cstddef>
#include <cstdint>
#include <cstring>
namespace rocell_diag {
class P4CorrectionOwner {
 public:
  ...
  void poll(Acquire& acquire,Write& write,Clock& clock,Evidence& evidence,
    ...
    if(state_==State::CapturingStart){
      ...
      if(!evidence("P4_CORRECTION_LEG_INTENT",pose)){fail("EVIDENCE_FAILED");return;}
  ...
  bool acknowledge(unsigned leg,const uint8_t* digest,Clock& clock){
    ...
    if(leg_==15){state_=State::Complete;reason_="P4_CORRECTION_COMPLETE";return true;}
