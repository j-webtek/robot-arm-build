// One-use, export-gated fixed 17-leg noncontact campaign.
#pragma once
 ...
 public:
  ...
  void poll(Acquire& acquire,Write& write,Clock& clock,Evidence& evidence,
    ...
    if(state_==State::CapturingStart){
      ...
      if(!evidence("AIR_TYPING_LEG_INTENT",pose)){fail("EVIDENCE_FAILED");return;}
    ...
    if(state_==State::Prewrite){
      ...
      if(!write(policy_.target_goals,uint16_t(20),uint8_t(1))){
  ...
  template<class Clock> bool acknowledge(unsigned leg,const uint8_t* digest,Clock& clock){
    ...
    if(leg_==AirTypingPolicy::legs-1){state_=State::Complete;reason_="AIR_TYPING_COMPLETE";return true;}
