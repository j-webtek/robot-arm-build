...
int main(int argc,char** argv){
  ...
  P4MidpointOwner owner;Clock clock;unsigned transmissions=0;
  uint16_t pos[7]={2047,2225,1890,2716,1978,2041,2047};
  ...
  for(unsigned leg=1;leg<=16;++leg){
    ...
    auto write=[&](uint8_t id,uint16_t target,uint16_t speed,uint8_t acc){
      ++transmissions;
      if(id!=15||target!=P4MidpointPolicy::targets[leg-1]||speed!=20||acc!=1)return false;
      if(mode=="delivery"&&leg==bad_leg)return false;
      goals[4]=target;pos[4]=target==1980?1978:target+2;
      if(mode=="no_benefit" && target==1945)pos[4]=1946;
      return true;
    };
    auto evidence=[&](const char* event,const ShoulderPreloadPose&){
      return !(mode=="evidence"&&leg==bad_leg&&std::strcmp(event,"P4_MIDPOINT_LEG_INTENT")==0);
    };
    for(unsigned i=0;i<7&&owner.state()!=P4MidpointOwner::State::Fault;++i){
    ...
    if((mode=="delivery"||mode=="drift"||mode=="evidence"||mode=="source"||mode=="prewrite"||mode=="timeout")&&leg==bad_leg){
      return owner.state()==P4MidpointOwner::State::Fault&&
        transmissions==leg-((mode=="delivery"||mode=="drift")?0:1)&&!owner.begin(reserve,clock)?0:2;
    }
    if(owner.state()!=P4MidpointOwner::State::AwaitExport)return 3;
    ...
    if(mode=="success"||mode=="no_benefit"){for(auto b:raw)std::printf("%02x",b);std::puts("");}
    ...
    if(mode=="expired"&&leg==bad_leg){
      clock.now+=30000001;
      return !owner.acknowledge(leg,digest,clock)&&owner.state()==P4MidpointOwner::State::Fault?0:8;
    }
  ...
  return owner.state()==P4MidpointOwner::State::Complete&&owner.total_writes()==16&&
    transmissions==16&&!owner.begin(reserve,clock)?0:10;
}
