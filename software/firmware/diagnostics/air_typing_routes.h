// Fixed export-gated, 17-leg air-typing route; no client-supplied targets.
#pragma once
#include "air_typing_owner.h"
#include <cstring>
#include <cstdio>
namespace rocell_diag {
template<class Crypto,class Services,class Clock,class Web> class AirTypingRoutes {
 public:
  AirTypingRoutes(Crypto& crypto,Services& services,Clock& clock,Web& web,
  ...
  void register_routes(){
    ...
    web_.on("/rocell/air-type/start",HTTP_POST,[this](){start();});
    web_.on("/rocell/air-type/status",HTTP_GET,[this](){status();});
    web_.on("/rocell/air-type/record",HTTP_GET,[this](){record();});
    web_.on("/rocell/air-type/receipt",HTTP_POST,[this](){receipt();});
    web_.on("/rocell/air-type/next",HTTP_POST,[this](){next();});
  }
  void poll(){
    auto acquire=[this](ShoulderPreloadPose& p){return services_.air_typing_sample(p);};
    auto write=[this](const uint16_t (&goals)[7],uint16_t speed,uint8_t acc){
      return services_.air_typing_write(goals,speed,acc);};
    auto evidence=[this](const char* event,const ShoulderPreloadPose& p){
      return services_.air_typing_evidence(event,p);};
    auto admitted=[this](){return services_.owned()&&services_.healthy();};
    owner_.poll(acquire,write,clock_,evidence,admitted);
  }
  bool claimed()const{return owner_.state()!=AirTypingOwner::State::New;}
 private:
  static int nibble(char c){return c>='0'&&c<='9'?c-'0':c>='a'&&c<='f'?c-'a'+10:-1;}
  bool empty(){return web_.args()==0;}
  void start(){
    const auto body=web_.arg("plain");
    if(web_.args()!=1||web_.argName(0)!="plain"||body!="AIR17"){
  ...
  void status(){
    ...
    char response[128];snprintf(response,sizeof(response),"%s|%u",
      owner_.state()==AirTypingOwner::State::AwaitExport?"AWAITING_EXPORT":owner_.reason(),owner_.leg());
  ...
  void receipt(){
    ...
    web_.send(200,"text/plain",owner_.state()==AirTypingOwner::State::Complete?"COMPLETE":response);
  }
  Crypto& crypto_;Services& services_;Clock& clock_;Web& web_;AirTypingOwner owner_;
