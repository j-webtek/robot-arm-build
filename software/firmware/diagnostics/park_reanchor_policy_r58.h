// r58 fixed visible shoulder step. This header has no route or bus write.
// It is staged as park_reanchor_policy.h only inside the reviewed r58 source.
#pragma once
#include "park_step_policy.h"

namespace rocell_diag {
struct ParkReanchorPolicy {
  static constexpr uint16_t source_goals[7]={2047,2389,1725,2907,1589,2040,2047};
  static constexpr uint16_t source_positions[7]={2047,2391,1724,2904,1591,2041,2047};
  static constexpr uint16_t target12=2413,target13=1701;
  static constexpr uint16_t expected12=2415,expected13=1700;
  static constexpr int maximum_travel=28;
  ...
  static bool endpoint(const ShoulderPreloadPose (&before)[3],
    ...
    return std::abs(int(last.position[1])-expected12)<=3&&
           std::abs(int(last.position[2])-expected13)<=3&&
           std::abs(int(last.position[1])-target12)<=3&&
           std::abs(int(last.position[2])-target13)<=3;
  }
};
}
