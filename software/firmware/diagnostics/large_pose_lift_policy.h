// Exact P0 -> T1 lift policy. Encoder-domain checks complement, but do not
// replace, the reviewed swept-geometry plan in LARGE_POSE_LADDER_PLAN.md.
#pragma once
#include "shoulder_characterization_policy.h"
#include <cstdlib>
namespace rocell_diag {
struct LargePoseLiftPolicy {
  static constexpr uint16_t source_goals[7]={2047,2413,1701,2907,1589,2040,2047};
  static constexpr uint16_t source_positions[7]={2047,2415,1700,2904,1591,2041,2047};
  static constexpr uint16_t target_goals[7]={2047,2348,1766,2907,1654,2040,2047};
  static constexpr int selected[3]={1,2,4};

  static bool is_selected(int joint){return joint==1||joint==2||joint==4;}
  ...
  static bool source(const ShoulderPreloadPose (&samples)[3],uint64_t now){
    ...
    for(int i=0;i<7;++i)
      if(current.goal[i]!=source_goals[i]||
         std::abs(int(current.position[i])-int(source_positions[i]))>2)return false;
    return true;
  }
