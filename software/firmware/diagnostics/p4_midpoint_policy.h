// Fixed sixteen-leg wrist campaign policy; not installed or exposed by a live route.
#pragma once
#include "shoulder_characterization_policy.h"
#include <cstdlib>
namespace rocell_diag {
struct P4MidpointPolicy {
  uint16_t source_goals[7]={2047,2217,1897,2711,1980,2040,2047};
  uint16_t source_positions[7]={2047,2225,1890,2716,1978,2041,2047};
  uint16_t target_goals[7]={2047,2217,1897,2711,1944,2040,2047};
  static constexpr uint16_t targets[16]={1944,1980,1945,1980,1945,1980,1944,1980,1945,1980,1944,1980,1944,1980,1945,1980};
  uint16_t baseline[7]={2047,2225,1890,2716,1978,2041,2047};
  ...
  bool source(const ShoulderPreloadPose (&samples)[3],uint64_t now){
    ...
    if(target_goals[4]<1980)for(const auto& sample:samples)
      if(sample.position[4]<1977 || sample.position[4]>1979)return false;
    for(int i=0;i<7;++i)
  ...
  bool unchanged_source(const ShoulderPreloadPose& current,
    ...
    if(target_goals[4]<1980 && (current.position[4]<1977 || current.position[4]>1979))return false;
    for(int i=0;i<7;++i)
