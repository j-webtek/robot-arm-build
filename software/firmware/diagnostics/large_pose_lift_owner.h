// One exact synchronized P0 -> T1 lift. One boot permits one write attempt;
// there is no retry, return, or continuation to another pose.
#pragma once
#include "large_pose_lift_policy.h"
#include <cstddef>
#include <cstdint>
#include <cstring>
namespace rocell_diag {
class LargePoseLiftOwner {
 public:
  ...
  void poll(Acquire& acquire,Write& write,Clock& clock,Evidence& evidence,
    ...
    if(state_==State::CapturingStart){
      ...
      if(count_<3)return;
      if(!LargePoseLiftPolicy::source(before_,clock.now_us())){
        fail("SOURCE_POSE_REJECTED");return;
      }
      if(!evidence("LARGE_POSE_T1_INTENT",pose)){fail("EVIDENCE_FAILED");return;}
      state_=State::Prewrite;reason_="PREWRITE";return;
    }
    if(state_==State::Prewrite){
      if(!LargePoseLiftPolicy::unchanged_source(pose,before_[2])||
         !LargePoseLiftPolicy::source(before_,clock.now_us())){
      ...
      ++writes_;
      if(!write(uint8_t(12),uint8_t(13),uint8_t(15),uint16_t(2348),
                uint16_t(1766),uint16_t(1654),uint16_t(20),uint8_t(1))){
    ...
    if(!LargePoseLiftPolicy::bounded_endpoint_sample(before_[2],pose)){
      fail("ENDPOINT_OUT_OF_BOUNDS");return;
    }
    if(count_<3)after_[count_++]=pose;
    else {after_[0]=after_[1];after_[1]=after_[2];after_[2]=pose;}
    if(count_<3||!LargePoseLiftPolicy::endpoint(before_,after_,sent_us_,clock.now_us()))return;
  ...
  bool mark_export_verified(bool verified){
    ...
    state_=State::Complete;reason_="LARGE_POSE_T1_RECORDED";return true;
  }
  ...
  size_t copy_result(const uint8_t (&boot)[16],uint8_t* out,size_t capacity)const{
    constexpr size_t required=10+16+6+8+1+7*(16+7*20);
    ...
    const char domain[]="RCLIFT0001";
    for(size_t i=0;i<sizeof(domain)-1;++i)put(uint8_t(domain[i]),1);
    for(auto byte:boot)put(byte,1);
    put(2348,2);put(1766,2);put(1654,2);put(sent_us_,8);put(writes_,1);
