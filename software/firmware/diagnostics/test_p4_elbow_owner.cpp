...
static ShoulderPreloadPose pose(unsigned index){
  ...
  if(after){sample.position[3]=2714;}
...
int main(int argc,char** argv){
  ...
  auto acquire=[&](ShoulderPreloadPose& out){
    ...
    if(mode=="source_rejected"&&acquisition==2)out.goal[3]++;
    if(mode=="prewrite_changed"&&acquisition==3)out.position[0]+=2;
    if(mode=="wrong_goal"&&acquisition==6)out.goal[3]++;
    if(mode=="passive_drift"&&acquisition==6)out.position[1]+=3;
    if(mode=="reverse"&&acquisition>=4)out.position[3]=2782;
  ...
  auto write=[&](uint8_t a,uint16_t x,
                 uint16_t speed,uint8_t acc){
    if(a!=14||x!=2711||speed!=20||acc!=1)return false;
    ++transmissions;return mode!="write_uncertain";
  };
  auto evidence=[&](const char* event,const ShoulderPreloadPose&){
    return !(mode=="evidence_failure"&&std::strcmp(event,"LARGE_POSE_P4E_INTENT")==0);};
  ...
  if(mode=="success"){
    uint8_t boot[16],record[1129];for(auto& byte:boot)byte=0xab;
