...
struct Services {
  ...
  bool large_pose_relief_write(uint8_t id,uint16_t target,uint16_t speed,uint8_t acc){
    assert(id==15&&target==P4MidpointPolicy::targets[writes]&&speed==20&&acc==1);
    goals[4]=target;pos[4]=target==1980?1978:target+2;++writes;return true;
  }
...
int main(){
  ...
  request("start","P4M16");assert(web.status==409);
