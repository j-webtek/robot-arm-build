...
#if defined(ROCELL_LOCAL_SHOULDER_STEP)
struct FakeESP {
...
int main(int argc,char** argv){
 ...
 assert(!rocellLocalSession&&st.reads==0&&st.writes==0);
 assert(!rocellLocalMemoryFits(0,1000000));assert(!rocellLocalMemoryFits(1000000,1));
 ...
 auto prepare=server.routes.at("/rocell/local-step/prepare");
 auto authorize=server.routes.at("/rocell/local-step/authorize");
 ...
 if(mode=="conflict"||mode=="missing_key"||mode=="unhealthy"||mode=="busy"||mode=="parameters"||mode=="low_heap"||mode=="fragmented"){
   assert(server.status==409||server.status==400);assert(!rocellLocalSession);
 }else{
   assert(server.status==200&&rocellLocalSession&&rocellLocalSettling);
   ...
   server.routes.at("/rocell/local-step/status")();assert(server.status==200);
   assert(rocellShoulderReserved&&rocellDiagnosticOwned);
 }
 std::cout<<"LOCAL_OBJECT_BYTES "<<sizeof(RocellLocalSession)<<" "<<sizeof(RocellLocalSettling)<<"\n";
