...
int main(int argc,char** argv){
  ...
  if(valid)assert(!ordinary.consume(token.data(),token.size(),1001,crypto,policy(),"reviewed-six-recovery"));
  // A signed six-count plan must also be rejected by the old five-count path.
  HoldPlanAdmission legacy(key,boot,nonce,1000,10001000);
  if(valid)assert(!legacy.consume(token.data(),token.size(),1001,crypto,policy(),"reviewed-six-recovery",true));
  for(int scenario=0;scenario<7;++scenario){
    bool healthy=true;GuardedBus bus;bus.healthy=&healthy;bus.automatic=false;
    bus.torque[3]=1;bus.goal[3]=2729;Clock clock;clock.tick=1001;RuntimeSink sink;
    ...
    HoldAuthenticatedRuntime<GuardedBus,Clock,RuntimeSink,Crypto,true,6> runtime(
      bus,clock,sink,crypto,key,boot,nonce,1000,10001000,policy(),
      "reviewed-six-recovery",healthy_runtime,&healthy);
