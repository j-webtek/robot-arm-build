// Authenticated fixed twelve-leg P4 repeat routes. No caller-supplied targets.
#pragma once
#include "p4_repeat_owner.h"
#include <cstring>
#include <cstdio>
namespace rocell_diag {
template<class Crypto,class Services,class Clock,class Web> class P4RepeatRoutes {
 public:
  P4RepeatRoutes(Crypto& crypto,Services& services,Clock& clock,Web& web,
  ...
  void register_routes(){
    ...
    web_.on("/rocell/p4-repeat/start",HTTP_POST,[this](){start();});
    web_.on("/rocell/p4-repeat/status",HTTP_GET,[this](){status();});
    web_.on("/rocell/p4-repeat/record",HTTP_GET,[this](){record();});
    web_.on("/rocell/p4-repeat/receipt",HTTP_POST,[this](){receipt();});
    web_.on("/rocell/p4-repeat/next",HTTP_POST,[this](){next();});
  }
  void poll(){
 ...
 private:
  ...
  void status(){
    ...
    char response[128];snprintf(response,sizeof(response),"%s|%u",owner_.state()==P4RepeatOwner::State::AwaitExport?"AWAITING_EXPORT":owner_.reason(),owner_.leg());
  ...
  void receipt(){
    ...
    web_.send(200,"text/plain",owner_.state()==P4RepeatOwner::State::Complete?"COMPLETE":response);
  }
  Crypto& crypto_;Services& services_;Clock& clock_;Web& web_;P4RepeatOwner owner_;
