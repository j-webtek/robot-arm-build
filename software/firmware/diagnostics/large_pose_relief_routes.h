// Authenticated exact measured T1 -> P1 route. The literal body "P1" selects the only
...
template<class Crypto,class Services,class Clock,class Web> class LargePoseReliefRoutes {
 public:
  LargePoseReliefRoutes(Crypto& crypto,Services& services,Clock& clock,Web& web,
  ...
  void register_routes(){
    ...
    web_.on("/rocell/large-pose-relief/start",HTTP_POST,[this](){start();});
    web_.on("/rocell/large-pose-relief/status",HTTP_GET,[this](){status();});
    web_.on("/rocell/large-pose-relief/record",HTTP_GET,[this](){record();});
    web_.on("/rocell/large-pose-relief/receipt",HTTP_POST,[this](){receipt();});
  }
  void poll(){
    auto acquire=[this](ShoulderPreloadPose& pose){return services_.large_pose_relief_sample(pose);};
    auto write=[this](uint8_t a,uint8_t b,uint16_t x,uint16_t y,
                      uint16_t speed,uint8_t acc){
      return services_.large_pose_relief_write(a,b,x,y,speed,acc);};
 ...
 private:
  ...
  void receipt(){
    ...
    web_.send(200,"text/plain","LARGE_POSE_P1_RECORDED");
  }
  Crypto& crypto_;Services& services_;Clock& clock_;Web& web_;LargePoseReliefOwner owner_;
  uint8_t boot_[16]={},record_[1131]={},digest_[32]={};char hex_[2263]={};
