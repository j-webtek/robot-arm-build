// Dedicated, one-write stylus-loading image. No generic JSON command parser.
// The only actuator command is a bounded opening of servo 17 over local USB.
#pragma once
...
class GripperLoadingBoard {
  ...
  void begin(){
    server.on("/rocell/gripper-loader/capabilities",HTTP_GET,[this](){
      ...
      snprintf(out,sizeof(out),
        "{\"schema\":\"rocell.gripper_loader.v1\",\"boot_id\":\"%s\","
        "\"one_open_write_max\":true,\"other_joint_writes\":false}",
 ...
 private:
  ...
  void dispatch(const char* line){
    ...
    if(attempted_||!snapshot_valid_){Serial.println("FAULT:ONE_USE_OR_NO_SNAP");return;}
    attempted_=true;
    Pose fresh{},check{};
    if(!reviewed_pose(snapshot_)||!sample(fresh)||!stable(snapshot_,fresh)||
      ...
      Serial.println("FAULT:PREWRITE_POSE");return;
    }
    before_=check;
    target_=uint16_t(check.position[6]-200);
    const int result=st.WritePosEx(17,target_,40,1); // One write; never retry.
    if(result!=1||st.Error!=0){Serial.println("FAULT:WRITE_UNCERTAIN");return;}
    sent_=true;sent_ms_=millis();
    Serial.printf("OPEN_SENT:%s:%u\n",rocellDiagnosticInstance,unsigned(target_));
  }
  Pose snapshot_{},before_{};
  char input_[64]{};size_t size_=0;uint16_t target_=0;uint32_t sent_ms_=0;
  bool snapshot_valid_=false,attempted_=false,sent_=false,finished_=false,verified_=false;
};
}
