// Authenticated, fixed-target re-anchor route. The HTTP body cannot supply a
// servo ID, target, speed, acceleration, or retry. Registration is inert.
#pragma once
#include "fixed_pair_reanchor_owner.h"
#include <cstring>
#include <cstdio>
namespace rocell_diag {
template<class Crypto,class Services,class Clock,class Web> class FixedPairReanchorRoutes {
 public:
  FixedPairReanchorRoutes(Crypto& crypto,Services& services,Clock& clock,Web& web,
  ...
  void register_routes(){
    ...
    web_.on("/rocell/reanchor/start",HTTP_POST,[this](){start();});
    web_.on("/rocell/reanchor/status",HTTP_GET,[this](){status();});
    web_.on("/rocell/reanchor/record",HTTP_GET,[this](){record();});
    web_.on("/rocell/reanchor/receipt",HTTP_POST,[this](){receipt();});
  }
  void poll(){
    auto acquire=[this](ShoulderPreloadPose& pose){return services_.reanchor_sample(pose);};
    auto write=[this](uint8_t a,uint8_t b,uint16_t x,uint16_t y,uint16_t speed,uint8_t acc){
      return services_.reanchor_write(a,b,x,y,speed,acc);
    };
    auto evidence=[this](const char* event,const ShoulderPreloadPose& pose){
      return services_.reanchor_evidence(event,pose);
    };
    auto admitted=[this](){return services_.owned()&&services_.healthy();};
    owner_.poll(acquire,write,clock_,evidence,admitted);
  }
  bool claimed()const{return owner_.state()!=FixedPairReanchorOwner::State::New;}
 private:
  ...
  void start(){
    ...
    bool ok=owner_.begin(reserve,clock_);
  ...
  void receipt(){
    ...
    for(unsigned i=0;i<32;++i){
      int hi=nibble(body[2*i]),lo=nibble(body[2*i+1]);
    ...
    web_.send(200,"text/plain","REANCHOR_VERIFIED");
  }
  Crypto& crypto_;Services& services_;Clock& clock_;Web& web_;
  FixedPairReanchorOwner owner_;
  uint8_t boot_[16]={},record_[1127]={},digest_[32]={};
  char hex_[2255]={};size_t size_=0;bool registered_=false;
};
}
