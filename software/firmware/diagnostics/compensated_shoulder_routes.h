...
template<class Access,class Clock,class Web> class CompensatedShoulderStepRoutes {
 public:
  CompensatedShoulderStepRoutes(Access& access,Clock& clock,Web& web):access_(access),clock_(clock),web_(web){}
  void register_routes(){
    if(registered_)return;registered_=true;
    web_.on("/rocell/compensated-step/status",HTTP_GET,[this](){
      if(web_.args()){send(400,"{}");return;}auto* s=access_();if(!s){send(409,"{}");return;}
      JsonDocument doc;doc["schema"]="rocell.compensated_step_status.v1";
    ...
    web_.on("/rocell/compensated-step/record",HTTP_GET,[this](){
      if(web_.args()){send(400,"{}");return;}auto* s=access_();if(!s){send(409,"{}");return;}
      if(!s->record()){send(404,"{}");return;}send(200,s->record());
    });
    web_.on("/rocell/compensated-step/authorize",HTTP_POST,[this](){accept(true);});
    web_.on("/rocell/compensated-step/receipt",HTTP_POST,[this](){accept(false);});
  }
 private:
  void accept(bool authorize){
    if(web_.args()!=1||web_.argName(0)!="plain"){send(400,"{}");return;}
    auto* s=access_();if(!s){send(409,"{}");return;}
    if(s->local_phase()!=(authorize?CompensatedStepPhase::Authorization:CompensatedStepPhase::Waiting)){send(409,"{}");return;}
