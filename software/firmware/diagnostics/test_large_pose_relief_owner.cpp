...
static ShoulderPreloadPose pose(unsigned index){
  ...
  if(after){sample.position[3]=2847;sample.position[4]=1717;}
...
int main(int argc,char** argv){
  ...
  auto acquire=[&](ShoulderPreloadPose& out){
    ...
    if(mode=="passive_drift"&&acquisition==6)out.position[2]+=3;
    if(mode=="reverse"&&acquisition>=4)out.position[4]=1640;
  ...
  auto write=[&](uint8_t a,uint8_t b,uint16_t x,uint16_t y,
    ...
    if(a!=14||b!=15||x!=2842||y!=1719||speed!=20||acc!=1)return false;
    ++transmissions;return mode!="write_uncertain";
  };
  auto evidence=[&](const char* event,const ShoulderPreloadPose&){
    return !(mode=="evidence_failure"&&std::strcmp(event,"LARGE_POSE_P1_INTENT")==0);};
