...
struct Services {
  ...
  bool large_pose_relief_write(uint8_t id,uint16_t target,uint16_t speed,uint8_t acc){
    assert(id==15&&target==P4RepeatPolicy::targets[writes]&&speed==20&&acc==1);
    goals[4]=target;pos[4]=target-3;++writes;return true;
  }
...
int main(){
  ...
  for(unsigned leg=1;leg<=12;++leg){
    ...
    assert(web.response==(leg==12?"COMPLETE":"READY|"+std::to_string(leg+1)));
  ...
  request("start","P4R12");assert(web.status==409);
  routes.poll();assert(services.writes==12);
}
