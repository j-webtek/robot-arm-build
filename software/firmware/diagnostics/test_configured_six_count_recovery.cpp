...
int main(int argc,char** argv){
  ...
  using Recovery=ConfiguredHoldRuntime<GuardedBus,Clock,ConfigServer,Client,Socket,Crypto,true,6>;
  for(int scenario=0;scenario<8;++scenario){
    Network net;current_network=&net;GuardedBus bus;Clock clock;clock.tick=1001;Crypto crypto;
    bool healthy=true;bus.healthy=&healthy;bus.automatic=false;bus.torque[3]=1;bus.goal[3]=2729;
    ...
    if(scenario==7){
      ...
      assert(runtime.initialize(bus,clock,crypto,key,boot,nonce,1000,10001000,8081,
          policy(),"r18-six-count-recovery",healthy_runtime,&healthy)==(scenario==0||scenario>=4));
    }
    fail_configured_allocation=0;fail_configured_listener=false;
    assert(!runtime.initialize(bus,clock,crypto,key,boot,nonce,1000,10001000,8081,
        policy(),"r18-six-count-recovery",healthy_runtime,&healthy));
