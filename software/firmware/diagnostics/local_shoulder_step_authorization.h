...
class LocalShoulderStepAuthorization {
 public:
  LocalShoulderStepAuthorization(const uint8_t (&key)[32],const uint8_t (&boot)[16],
  ...
  bool verify(const uint8_t* token,size_t length,uint64_t now,Crypto& crypto,
    ...
    if(deserializeJson(proposed,plan.bytes,plan.length)||!proposed["targets"][0].is<uint16_t>())return false;
    ...
    const int step=int(current.position[1])-int(proposed["targets"][0].as<uint16_t>());
    if(step<12||step>24||!contract_.prepare(current,unsigned(step),now))return false;
    ...
    expected["schema"]="rocell.local_shoulder_step.v1";expected["boot_id"]=boot_;
    ...
    auto targets=expected["targets"].to<JsonArray>();targets.add(contract_.targets[0]);targets.add(contract_.targets[1]);
    expected["speed"]=20;expected["acceleration"]=1;expected["maximum_target_packets"]=1;
  ...
  const LocalShoulderStepContract* contract()const{return verified_?&contract_:nullptr;}
 private:
  StartEnvelopeGate gate_;LocalShoulderStepContract contract_;
