...
struct Bus {
 ...
 uint16_t positions[7]={2047,2429,1688,2904,1591,2041,2047};
 uint16_t goals[7]={2047,2419,1695,2907,1589,2040,2047};
 ...
 void SyncWritePosEx(uint8_t* ids,uint8_t n,int16_t* targets,uint16_t* speed,uint8_t* acc){
   ...
   assert(targets[0]==2405&&targets[1]==1709);
...
int main(int argc,char** argv){
 ...
 using Session=rocell_diag::LocalShoulderStepSession<Crypto>;
 auto session=std::make_unique<Session>(crypto,key,boot,nonce,1,30000001,"local-step-1");
 auto admitted=[](){return true;};using P=rocell_diag::LocalStepPhase;
 for(int i=0;i<100;++i){
 ...
 if(fault&&(bus.mode=="short"||bus.mode=="neighbor")){
