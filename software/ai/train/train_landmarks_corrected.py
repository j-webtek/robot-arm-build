...
def run(corrective):
    ...
    out=AI/'results'/('landmark_corrected_v0_'+('corrected' if corrective else 'control'))
    ...
    device='cuda' if torch.cuda.is_available() else 'cpu';model=KeyboardLandmarkNet().to(device)
    ...
    def loss(output,points,visible,corrected):
        ...
        if not corrected:return loc+vis
        ...
        return loc+balanced_visibility_loss(output['visibility_logits'],visible)+coordinate_loss
    out.mkdir(parents=True);history=[];best=float('inf');selected=0
    for epoch in range(m['epochs']):
        model.train();total=0
        for bx,by,bv in loader:
            optimizer.zero_grad();value=loss(model(bx.to(device).float()/255),by.to(device),bv.to(device),corrective);value.backward();optimizer.step();total+=value.item()*len(bx)
        model.eval();dev_total=0
        with torch.no_grad():
            for start in range(0,len(dx),32):dev_total+=loss(model(dx[start:start+32].to(device).float()/255),dy[start:start+32].to(device),dv[start:start+32].to(device),True).item()*len(dx[start:start+32])
    ...
    (AI/'eval'/('landmark_corrected_v0_'+('corrected' if corrective else 'control')+'_scorecard.json')).write_text(json.dumps(report,indent=2)+'\n')
...
if __name__=='__main__':
    control=run(False);candidate=run(True)
    checks={c:dict(mean=candidate['summaries'][c]['landmark']['mean_mm']<control['summaries'][c]['landmark']['mean_mm'],
        tail=candidate['summaries'][c]['landmark']['tail']<=control['summaries'][c]['landmark']['tail'],
        yaw=candidate['summaries'][c]['landmark']['yaw_p95']<=control['summaries'][c]['landmark']['yaw_p95']*1.1) for c in control['summaries']}
    visibility_pass=candidate['false_visible_occluded']<control['false_visible_occluded'] and candidate['true_visible_clear']/candidate['clear_corners']>=.9
    report=dict(scope='MATCHED_DEVELOPMENT_CORRECTIVE_LOSS',checks=checks,visibility_pass=visibility_pass,
        corrective_rule_passed=all(all(c.values()) for c in checks.values()) and visibility_pass,
        control_checkpoint_sha256=control['checkpoint_sha256'],candidate_checkpoint_sha256=candidate['checkpoint_sha256'],
        control_report_sha256=hashlib.sha256((AI/'eval/landmark_corrected_v0_control_scorecard.json').read_bytes()).hexdigest(),
        candidate_report_sha256=hashlib.sha256((AI/'eval/landmark_corrected_v0_corrected_scorecard.json').read_bytes()).hexdigest(),
        hardware_writes=0,physical_movements=0,qualification_installed=False,
        limitations=['Combined coordinate and visibility intervention cannot isolate individual effects',
                    'Both arms selected by the same corrected development loss; reused data, one seed',
                    'Improving failed landmark control is not sufficient to replace existing pose baseline'])
    (AI/'eval/landmark_corrected_v0_comparison.json').write_text(json.dumps(report,indent=2)+'\n')
