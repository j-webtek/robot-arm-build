...
def run():
    ...
    def loss(output,points,visible):
    ...
    for epoch in range(m['epochs']):
        ...
        for bx,by,bv in loader:
            optimizer.zero_grad();value=loss(model(bx.to(device).float()/255),by.to(device),bv.to(device));value.backward();optimizer.step();total+=value.item()*len(bx)
        model.eval();dev_total=0
        with torch.no_grad():
            for start in range(0,len(dx),32):dev_total+=loss(model(dx[start:start+32].to(device).float()/255),dy[start:start+32].to(device),dv[start:start+32].to(device)).item()*len(dx[start:start+32])
    ...
    (AI/'eval/landmark_v0_scorecard.json').write_text(json.dumps(report,indent=2)+'\n')
