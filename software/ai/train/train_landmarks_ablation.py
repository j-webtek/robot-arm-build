...
def run(mode):
    path=AI/'train/landmark_ablation_v0_plan.json';m=json.loads(path.read_text())
    ...
    out=AI/'results'/('landmark_ablation_v0_'+mode)
    ...
    for epoch in range(m['epochs']):
        ...
        for bx,by,bv in loader:
            optimizer.zero_grad();value=loss(model(bx.to(device).float()/255),by.to(device),bv.to(device),mode);value.backward();optimizer.step();total+=value.item()*len(bx)
    ...
    (AI/'eval'/('landmark_ablation_v0_'+mode+'_scorecard.json')).write_text(json.dumps(report,indent=2)+'\n')
...
if __name__=='__main__':
    results={name:run(name) for name in ('coordinate_only','visibility_only')}
    reference=json.loads((AI/'eval/landmark_corrected_v0_control_scorecard.json').read_text())
    checks={}
    ...
    report=dict(scope='MATCHED_DEVELOPMENT_LOSS_ABLATION',checks=checks,
        report_hashes={name:hashlib.sha256((AI/f'eval/landmark_ablation_v0_{name}_scorecard.json').read_bytes()).hexdigest() for name in results},
        hardware_writes=0,physical_movements=0,qualification_installed=False,
        limitations=['Reuses frozen previous control; same data/order/seed/budget/selection, GPU execution not bitwise guaranteed',
    ...
    (AI/'eval/landmark_ablation_v0_comparison.json').write_text(json.dumps(report,indent=2)+'\n')
