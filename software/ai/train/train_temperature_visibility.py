...
def run(mode):
    path=AI/'train/landmark_temperature_v0_plan.json';m=json.loads(path.read_text())
    ...
    out=AI/'results'/('landmark_temperature_v0_'+mode)
    ...
    device='cuda' if torch.cuda.is_available() else 'cpu';model=TemperatureVisibilityNet(mode).to(device)
    ...
    (AI/'eval'/('landmark_temperature_v0_'+mode+'_scorecard.json')).write_text(json.dumps(report,indent=2)+'\n')
...
if __name__=='__main__':
    results={name:run(name) for name in ('t1','t05')}
    reference=results['t1']
    checks={}
    ...
    report=dict(scope='MATCHED_TEMPERATURE_VISIBILITY',checks=checks,
        report_hashes={name:hashlib.sha256((AI/f'eval/landmark_temperature_v0_{name}_scorecard.json').read_bytes()).hexdigest() for name in results},
        hardware_writes=0,physical_movements=0,qualification_installed=False,
        limitations=['Retrained matched temperature1/0.5 heads; identical parameters and seed; GPU execution not bitwise guaranteed',
    ...
    (AI/'eval/landmark_temperature_v0_comparison.json').write_text(json.dumps(report,indent=2)+'\n')
