...
def test_identical_initialization_and_constant_field_agreement():
    torch.manual_seed(17);a=TemperatureVisibilityNet('t1')
    torch.manual_seed(17);b=TemperatureVisibilityNet('t05')
...
def test_local_head_receives_feature_gradients():
    torch.manual_seed(17);m=TemperatureVisibilityNet('t05');x=torch.rand(1,3,192,256)
...
def test_weighted_provenance_and_decisions():
    path=AI/'train/landmark_temperature_v0_plan.json';plan=json.loads(path.read_text())
    for f,h in plan['file_sha256'].items():
        assert hashlib.sha256((ROOT/f).read_bytes()).hexdigest()==h
    control=json.loads((AI/'eval/landmark_temperature_v0_t1_scorecard.json').read_text())
    comparison=json.loads((AI/'eval/landmark_temperature_v0_comparison.json').read_text())
    for mode in ('t1','t05'):
        p=AI/f'eval/landmark_temperature_v0_{mode}_scorecard.json';r=json.loads(p.read_text())
...
def test_parity_reports():
    ...
    for mode in ('t1','t05'):
        ...
        r=json.loads((AI/f'eval/temperature_visibility_{mode}_parity_report.json').read_text())
        ...
        retained=json.loads((AI/f'eval/landmark_temperature_v0_{mode}_scorecard.json').read_text())
