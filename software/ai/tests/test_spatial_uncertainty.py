...
from vision.evaluate_spatial_uncertainty import bin_index
...
def test_radius_support_abstention_and_metrics():
    ...
    r=json.loads((AI/'eval/spatial_uncertainty_v0_report.json').read_text())
...
def test_shift_direction_padding_and_inverse_projection():
    import numpy as np
    ...
    right=np.asarray(shift_image(image,1,0));down=np.asarray(shift_image(image,0,1))
    assert np.array_equal(right[:,1:],pixels[:,:-1])
    assert np.array_equal(right[:,0],pixels[:,0])
    assert np.array_equal(down[1:],pixels[:-1])
    assert np.array_equal(down[0],pixels[0])
    assert image.tobytes()==pixels.tobytes()
    for dx,dy in [(1,0),(-1,0),(0,1),(0,-1)]:
        corrected=undo_shift((235+dx*610/128,154+dy*457/96,3.14),dx,dy)
        assert corrected==(235,154,3.14)
