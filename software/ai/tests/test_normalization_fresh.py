...
def test_pixel_transform_bounds_and_no_mutation():
    ...
    assert gain==1 and result.getpixel((0,0))==(255,255,255)
    for value in (128,129,200):
        image=Image.new('RGB',(128,96),(value,value,value))
        result,gain=normalize(image)
        assert gain==1 and result.tobytes()==image.tobytes()
    result,gain=normalize(Image.new('RGB',(128,96),(127,127,127)))
    assert gain>1


def test_frozen_provenance_cases_and_acceptance():
    path=AI/'eval/normalization_fresh_v0.manifest.json'
    m=json.loads(path.read_text());r=json.loads((AI/'eval/normalization_fresh_v0_scorecard.json').read_text())
    ...
    assert len(r['cases'])==3500
    for condition in m['conditions']:
        cases=[c for c in r['cases'] if c['condition']==condition]
        assert [c['seed'] for c in cases]==list(range(21000000,21000500))
...
def test_bypass_accounting():
    r=json.loads((AI/'eval/normalization_fresh_v0_scorecard.json').read_text())
    for condition,count in r['corrected_counts'].items():
        rows=[c for c in r['cases'] if c['condition']==condition]
        assert count==sum(c['gain']!=1 for c in rows)
        for c in rows:
            if c['gain']==1:assert c['arms']['control']==c['arms']['normalized']
    assert r['corrected_counts']['standard']==0
    assert r['corrected_counts']['appearance_shift']==0
    assert r['corrected_counts']['darkened_standard']==500
