...
from vision.evaluate_disagreement_fresh import bin_index
...
def test_radius_support_abstention_and_metrics():
    ...
    r=json.loads((AI/'eval/disagreement_fresh_v0_report.json').read_text())
    ...
    assert len(r['fitting_cases'])==7000 and len(r['evaluation_cases'])==3500
...
def test_group_criteria_do_not_count_abstentions_as_success():
    r=json.loads((AI/'eval/disagreement_fresh_v0_report.json').read_text())
    groups=[]
    for seed in range(25000000,25000500):
    ...
    assert passed==r['evaluation_passed']


def test_fresh_seed_ranges():
    r=json.loads((AI/'eval/disagreement_fresh_v0_report.json').read_text())
    calibration={c['seed'] for c in r['fitting_cases']}
    evaluation={c['seed'] for c in r['evaluation_cases']}
    assert calibration==set(range(24000000,24001000))
    assert evaluation==set(range(25000000,25000500))
