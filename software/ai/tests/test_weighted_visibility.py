...
def test_identical_initialization_and_constant_field_agreement():
    torch.manual_seed(17);a=WeightedVisibilityNet('hard')
    torch.manual_seed(17);b=WeightedVisibilityNet('weighted')
...
def test_local_head_receives_feature_gradients():
    torch.manual_seed(17);m=WeightedVisibilityNet('weighted');x=torch.rand(1,3,192,256)
...
def test_weighted_provenance_and_decisions():
    path=AI/'train/landmark_weighted_visibility_v0_plan.json';plan=json.loads(path.read_text())
    for f,h in plan['file_sha256'].items():
        assert hashlib.sha256((ROOT/f).read_bytes()).hexdigest()==h
    control=json.loads((AI/'eval/landmark_weighted_visibility_v0_hard_scorecard.json').read_text())
    comparison=json.loads((AI/'eval/landmark_weighted_visibility_v0_comparison.json').read_text())
    for mode in ('hard','weighted'):
        p=AI/f'eval/landmark_weighted_visibility_v0_{mode}_scorecard.json';r=json.loads(p.read_text())
...
def test_parity_reports():
    import numpy as np
    from vision.diagnose_local_visibility_parity import compare
    for mode in ('hard','weighted'):
        p=AI/f'eval/weighted_visibility_{mode}_parity_plan.json';m=json.loads(p.read_text())
        r=json.loads((AI/f'eval/weighted_visibility_{mode}_parity_report.json').read_text())
        ...
        for variant in ('cpu_32','cuda_1','cuda_32'):
            assert compare(runs['cpu_1'],runs[variant])==r['comparisons']['cpu_1_vs_'+variant]
        retained=json.loads((AI/f'eval/landmark_weighted_visibility_v0_{mode}_scorecard.json').read_text())
        assert r['pixels_sha256']==retained['development_pixels_sha256']
        assert np.array_equal(runs['cuda_32']['probabilities'],[c['visibility_probabilities'] for c in retained['cases']])
