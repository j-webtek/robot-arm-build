...
def run():
    ...
    model=KeyboardLandmarkNet();model.load_state_dict(torch.load(ck,map_location='cpu',weights_only=True));model.eval();torch.set_num_threads(4)
    ...
    training=json.loads((AI/'eval/landmark_v0_scorecard.json').read_text())
    report=dict(scope='RETAINED_DEVELOPMENT_DIAGNOSTIC',manifest_sha256=hashlib.sha256(path.read_bytes()).hexdigest(),rows=rows,summaries=summaries,
    ...
    (AI/'eval/landmark_diagnostic_v0_report.json').write_text(json.dumps(report,indent=2)+'\n')
