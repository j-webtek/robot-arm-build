...
from vision.evaluate_normalized_radius import evaluate
...
def run():
    path=AI/'eval/normalized_uncertainty_v0.manifest.json'
    ...
    out=AI/'results/normalized_uncertainty_v0_full.json'
    if out.exists(): raise ValueError('preserve prior full evidence')
    out.write_bytes((json.dumps(full,indent=2)+'\n').encode())
    (AI/'eval/normalized_uncertainty_v0_scorecard.json').write_bytes((json.dumps(report,indent=2)+'\n').encode())
