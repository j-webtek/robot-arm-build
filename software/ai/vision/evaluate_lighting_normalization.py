...
def normalize(image):
    ...
    gain=float(np.clip(220/max(upper,1),.5,2.5))
    result=Image.fromarray(np.clip(np.rint(pixels*gain),0,255).astype(np.uint8))
    return result,gain


def run():
    path=AI/'eval/lighting_normalization_v0.manifest.json';m=json.loads(path.read_text())
    ...
    report=dict(scope='DEVELOPMENT_ONLY',manifest_sha256=hashlib.sha256(path.read_bytes()).hexdigest(),
        ...
        limitations=['Fixed global gain from image p95 luminance; no truth, placement, camera calibration or visibility input',
    ...
    (AI/'eval/lighting_normalization_v0_scorecard.json').write_bytes((json.dumps(report,indent=2)+'\n').encode())
