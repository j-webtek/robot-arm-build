...
def run():
    path=AI/'eval/disagreement_fresh_v0.manifest.json';m=json.loads(path.read_text())
    ...
    def measure(start,count):
        ...
        for seed in range(start,start+count):
            for condition in m['conditions']:
                ...
                variants=[image]+[ImageEnhance.Brightness(image).enhance(f) for f in m['perturbation_factors']]
        ...
        return rows
    fitting=measure(*m['calibration_groups']);radii=[];supports=[]
    ...
    # All radii fixed before evaluation of the separate fresh evaluation groups.
    evaluation=measure(*m['evaluation_groups'])
    for r in evaluation:
    ...
    groups=[]
    for seed in range(m['evaluation_groups'][0],sum(m['evaluation_groups'])):
    ...
    report=dict(scope='FRESH_SYNTHETIC_CALIBRATION_EVALUATION',manifest_sha256=hashlib.sha256(path.read_bytes()).hexdigest(),
        radii_mm=radii,bin_support_groups=supports,fitting_cases=fitting,evaluation_cases=evaluation,metrics=metrics,
        accepted_groups=groups,group_coverage=group_coverage,group_containment=group_fit,evaluation_passed=passed,
        hardware_writes=0,physical_movements=0,qualification_installed=False,
        limitations=['Fresh24M calibration and25M evaluation; seed ranges consumed by this study; same synthetic renderer, not physical deployment evidence',
                    'Perturbation stability may miss shared model bias; bins are not calibrated deployment confidence',
                    'Oracle geometry is scoring-only; abstained examples never counted as successful coverage',
                    'No runtime installation; empirical conditional coverage is not a formal population guarantee'])
    (AI/'eval/disagreement_fresh_v0_report.json').write_text(json.dumps(report,indent=2)+'\n')
