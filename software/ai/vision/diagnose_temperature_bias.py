...
def run():
    path=AI/'eval/temperature_bias_v0.manifest.json';m=json.loads(path.read_text())
    ...
    training=json.loads((AI/'eval/landmark_temperature_v0_t05_scorecard.json').read_text())
    decision_groups={}
    ...
    for group in ('clear','partial','hidden'):
        selected=[r for r in rows if r['visibility_class']==group]
        coordinate_summary[group]={key:dict(mean=float(np.mean([r[key] for r in selected])),p95=float(np.percentile([r[key] for r in selected],95))) for key in ('soft_error_px','sharp_error_px','peak_error_px')}
    report=dict(coordinate_summary=coordinate_summary,decision_groups=decision_groups,threshold_margins=margins,scope='RETAINED_DEVELOPMENT_DIAGNOSTIC',manifest_sha256=hashlib.sha256(path.read_bytes()).hexdigest(),rows=rows,summaries=summaries,
    ...
    out=AI/'eval/temperature_bias_v0_report.json'
    if out.exists():raise FileExistsError(out)
    out.write_text(json.dumps(report,indent=2)+'\n')
    print(json.dumps(dict(coordinate_summary=coordinate_summary,threshold_margins=margins),indent=2))
