"""Matched 25-percent dark-sample augmentation at lower learning rate; development only."""
...
if __name__=='__main__':
    plan_path=AI_DIR/'train/brightness_reduced_v0_plan.json'
    plan=json.loads(plan_path.read_text())
    results=[]
    for name,augment in plan['arms'].items():
        results.append(train(plan_path,AI_DIR/'results'/('brightness_reduced_v0_'+name),augment))
    ...
    report['limitations'].append('Fixed brightness 0.5 is reused from diagnostic; paired development selection only; control duplicates images for equal optimizer steps; candidate darkens extra copies for alternating seed groups (25 percent of all training images); both learning rates 0.00005')
    (AI_DIR/'eval/brightness_reduced_v0_scorecard.json').write_bytes((json.dumps(report,indent=2)+'\n').encode())
