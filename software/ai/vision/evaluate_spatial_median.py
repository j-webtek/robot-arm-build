...
def median_pose(poses):
    reference=poses[0][2]
    offsets=[math.atan2(math.sin(p[2]-reference),math.cos(p[2]-reference)) for p in poses]
    yaw=reference+float(np.median(offsets))
    return (float(np.median([p[0] for p in poses])),float(np.median([p[1] for p in poses])),
...
def run():
    ...
    (AI/'eval/spatial_median_v0_scorecard.json').write_bytes((json.dumps(report,indent=2)+'\n').encode())
