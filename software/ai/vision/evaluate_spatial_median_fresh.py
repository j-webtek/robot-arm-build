...
def run():
    path=AI/'eval/spatial_median_fresh_v0.manifest.json';m=json.loads(path.read_text())
    ...
    cases=[];digests={a:hashlib.sha256() for a in ('control','median')}
    for seed in range(m['seed_start'],m['seed_start']+m['seed_count']):
        for condition in m['conditions']:
            ...
            averaged=median_pose(corrected)
            ...
            for arm,pose in [('control',corrected[0]),('median',averaged)]:
    ...
    for condition in m['conditions']:
        ...
        a=summaries[condition]['control'];b=summaries[condition]['median']
    ...
    checks['overall']={'mean_improves':bool(np.mean([r['arms']['median']['mean_mm'] for r in cases])<np.mean([r['arms']['control']['mean_mm'] for r in cases]))}
    corrected_counts={c:sum(r['gain']!=1 for r in cases if r['condition']==c) for c in m['conditions']}
    report=dict(corrected_counts=corrected_counts,scope='FRESH_SYNTHETIC_ESTIMATOR_EVALUATION',manifest_sha256=hashlib.sha256(path.read_bytes()).hexdigest(),
        ...
        limitations=['Fresh26M seed groups; same synthetic renderer, no physical qualification; consumed after this evaluation',
                    'Five model passes; synthetic shift inverse is not measured camera calibration',
                    'Coordinate-wise center median and median wrapped yaw offsets from base; fixed before scoring'])
    (AI/'eval/spatial_median_fresh_v0_scorecard.json').write_bytes((json.dumps(report,indent=2)+'\n').encode())
