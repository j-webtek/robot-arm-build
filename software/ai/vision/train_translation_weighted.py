...
def train(plan_path, output, weights):
    size=[128,96]
...
if __name__=='__main__':
    plan_path=AI_DIR/'train/translation_weighted_v0_plan.json'
    plan=json.loads(plan_path.read_text())
    results=[]
    for name,weights in plan['arms'].items():
        results.append(train(plan_path,AI_DIR/'results'/('translation_weighted_v0_'+name),weights))
    ...
    (AI_DIR/'eval/translation_weighted_v0_scorecard.json').write_bytes((json.dumps(report,indent=2)+'\n').encode())
