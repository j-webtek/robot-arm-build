"""Instrumented synthetic renderer for landmark labels; keeps original RGB pixels."""
...
def _render_with_mask(seed: int, catalog: NominalTargetCatalog, *, domain: str = "standard",
           photo_texture: Image.Image | None = None) -> tuple[Image.Image, tuple[float, float, float], Image.Image]:
    ...
    if rng.random() < 0.55:
        ...
        draw.line((*upper, *lower), fill=(105, 119, 128), width=width)
        mask_draw.line((*upper, *lower), fill=255, width=width)
    if rng.random() < 0.6:
        y = rng.choice((rng.randint(4, 25), rng.randint(164, 188)))
        points = (rng.randint(-30, 20), y, rng.randint(235, 290), y + rng.randint(-7, 7))
        width = rng.randint(3, 6)
        draw.line(points, fill=(220, 165, 22), width=width)
        mask_draw.line(points, fill=255, width=width)
    if rng.random() < 0.35:
        x = rng.randint(35, 225)
        points = (x, -15, x + rng.randint(-25, 25), rng.randint(45, 110))
        width = rng.randint(4, 12)
        draw.line(points, fill=(17, 20, 24), width=width)
        mask_draw.line(points, fill=255, width=width)
    if rng.random() < 0.22:
    ...
    return image, (center[0], center[1], angle), mask
...
def render_landmarks(seed, catalog, *, domain="standard"):
    """Semantic case corners, ordered by local (-x,-y),(+x,-y),(+x,+y),(-x,+y).

    Occlusion fractions cover a radius2 pixel disk against drawn foreground masks.
    These are synthetic geometric labels; blur/low contrast are not visibility confidence.
    Photo textures are deliberately unsupported because case boundaries are approximate.
    """
    image, pose, mask = _render_with_mask(seed, catalog, domain=domain)
    corners = _rectangle_points(pose[:2], KEYBOARD_HALF_SIZE_MM, pose[2])
    labels = []
    for index, (x, y) in enumerate(corners):
        samples = [(round(x)+dx, round(y)+dy) for dy in range(-2,3) for dx in range(-2,3) if dx*dx+dy*dy<=4]
        inside = [(u,v) for u,v in samples if 0<=u<IMAGE_SIZE[0] and 0<=v<IMAGE_SIZE[1]]
        visible = sum(mask.getpixel((u,v))==0 for u,v in inside)
        labels.append(dict(corner_index=index,x_px=x,y_px=y,in_frame_fraction=len(inside)/len(samples),
                           unoccluded_fraction=visible/len(samples)))
    return image, dict(seed=seed,domain=domain,pose=list(pose),landmarks=labels,
                      frame="synthetic_image_pixels",image_size=list(IMAGE_SIZE),
                      visibility_semantics="geometric_foreground_mask_only",qualification_installed=False), mask
