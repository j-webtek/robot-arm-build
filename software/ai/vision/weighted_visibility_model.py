...
class WeightedVisibilityNet(LocalVisibilityNet):
    def __init__(self,mode):
        super().__init__('local')
        if mode not in ('hard','weighted'):raise ValueError(mode)
        self.pooling_mode=mode

    def forward(self,image):
        if self.pooling_mode=='hard':return super().forward(image)
        ...
        weights=h.flatten(2).softmax(-1).detach()
