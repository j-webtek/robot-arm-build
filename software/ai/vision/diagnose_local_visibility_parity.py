...
def run():
    p=AI/'eval/local_visibility_parity_plan.json';m=json.loads(p.read_text())
    ...
    x=np.stack(pixels);training=json.loads((AI/'eval/landmark_local_visibility_v0_local_scorecard.json').read_text())
    pixel_hash=hashlib.sha256(x.tobytes()).hexdigest();assert pixel_hash==training['development_pixels_sha256']
    runs={}
    for device in ('cpu','cuda'):
        model=LocalVisibilityNet('local').to(device);model.load_state_dict(torch.load(ck,map_location=device,weights_only=True));model.eval()
        for batch in (1,32):
    ...
    out=AI/'eval/local_visibility_parity_report.json'
