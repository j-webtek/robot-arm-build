"""Development-only paired pixel-refinement hypothesis; never qualification."""
...
def run():
    path=AI/'eval/local_refinement_v0.manifest.json'
    ...
    for seed in range(m['seed_start'],m['seed_start']+m['seed_count']):
        ...
        for condition in m['conditions']:
            ...
            for method in m['methods']:
                points=predicted if method=='baseline' else [refine(image,p) for p in predicted]
    ...
    result=dict(scope='DEVELOPMENT_ONLY',manifest_sha256=hashlib.sha256(path.read_bytes()).hexdigest(),
        ...
        limitations=['Development selection evidence only; no calibration/evaluation seeds accessed',
                    ...
                    'Edge centroid in 9x9 patch with Gaussian sigma 2 pixels and maximum 1 mm correction',
                    'Predicted coordinates and pixels only at inference; known target identity assumed'])
    (AI/'eval/local_refinement_v0_scorecard.json').write_bytes((json.dumps(result,indent=2)+'\n').encode())
