...
def run(mode):
    ...
    x=np.stack(pixels);training=json.loads((AI/f'eval/landmark_weighted_visibility_v0_{mode}_scorecard.json').read_text())
    pixel_hash=hashlib.sha256(x.tobytes()).hexdigest();assert pixel_hash==training['development_pixels_sha256']
    runs={}
    for device in ('cpu','cuda'):
        model=WeightedVisibilityNet(mode).to(device);model.load_state_dict(torch.load(ck,map_location=device,weights_only=True));model.eval()
        for batch in (1,32):
    ...
    out=AI/f'eval/weighted_visibility_{mode}_parity_report.json'
    if out.exists():raise FileExistsError(out)
    out.write_text(json.dumps(report,indent=2)+'\n');print(json.dumps(comparisons,indent=2))
if __name__=='__main__':
    for mode in ('hard','weighted'):run(mode)
