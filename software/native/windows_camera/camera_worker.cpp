...
void campaign(Receipt& receipt, ComPtr<IMFActivate>& selected,
  ...
  const uint64_t deadline = GetTickCount64() + o.max_ms;
  if (o.operation == "capture") validate_output(o.output);
  receipt.devices = enumerate_metadata();
  if (o.operation == "inventory") return;
  // Re-resolve the exact opaque endpoint immediately before activation. No
  // friendly-name or device-order fallback and no synthetic USB unit facts.
...
struct IdentityOptions {
  std::wstring endpoint;
  rocell::identity::Limits limits;
};
IdentityOptions parse_identity_options(const std::vector<std::wstring>& arguments) {
  std::map<std::wstring, std::wstring> values;
  require(arguments.size() == 6, "IDENTITY_ARGUMENT_COUNT");
  for (std::size_t i = 0; i < arguments.size(); i += 2) {
    require(values.emplace(arguments[i], arguments[i + 1]).second, "IDENTITY_DUPLICATE_ARGUMENT");
  }
  require(values.count(L"--endpoint") && values.count(L"--max-ms") && values.count(L"--max-parents"), "IDENTITY_ARGUMENT_NAMES");
  IdentityOptions result;
  result.endpoint = values.at(L"--endpoint");
  require(!result.endpoint.empty() && result.endpoint.size() <= 4096, "IDENTITY_ENDPOINT_BOUNDS");
  result.limits.duration_ms = static_cast<std::uint32_t>(number(values.at(L"--max-ms"), 100, 30000));
  result.limits.max_parent_nodes = static_cast<std::uint32_t>(number(values.at(L"--max-parents"), 0, 16));
  return result;
}

int self_test() {
  // No COM, MF, USB, capture APIs or filesystem writes in this branch.
  require(quote(std::string("a\\\"\nb")) == "\"a\\\\\\\"\\u000ab\"", "SELF_TEST_JSON");
  require(rows_fit(16, 0, 8, 4, 2), "SELF_TEST_ROWS");
  require(rows_fit(16, 8, -8, 4, 2), "SELF_TEST_NEGATIVE_ROWS");
  require(!rows_fit(15, 0, 8, 4, 2), "SELF_TEST_SHORT_FRAME");
  require(!rows_fit(16, 0, -8, 4, 2), "SELF_TEST_BAD_ORIGIN");
  const auto inventory = parse_options({L"inventory", L"--max-ms", L"5000"});
  require(inventory.operation == "inventory", "SELF_TEST_INVENTORY");
  unsigned denied = 0;
  for (const auto& args : std::vector<std::vector<std::wstring>>{
      {L"inventory", L"--max-ms", L"5000", L"--endpoint", L"hidden-open"},
      {L"probe", L"--max-ms", L"5000"},
      {L"probe", L"--max-ms", L"5000", L"--endpoint", L"x", L"--max-ms", L"6000"},
      {L"inventory", L"--max-ms", L"NaN"}}) {
    try { parse_options(args); } catch (const std::exception&) { ++denied; }
  }
  require(denied == 4, "SELF_TEST_UNKNOWN_ARGUMENT");
  require(parse_settings(L"exposure,-5,manual;gain,2,auto").size() == 2, "SELF_TEST_CONTROLS");
  try { parse_settings(L"focus,4,manual"); } catch (const std::exception&) { ++denied; }
  require(denied == 5, "SELF_TEST_UNSUPPORTED_CONTROL");
  Mode first;
  first.width = 4; first.height = 2; first.fps_n = 9; first.fps_d = 1;
  first.guid = MFVideoFormat_YUY2;
  Mode alternate = first; alternate.fps_n = 18; alternate.fps_d = 2;
  require(first.matches(alternate), "SELF_TEST_EQUIVALENT_RATE");
  alternate.fps_n = 20;
  require(!first.matches(alternate), "SELF_TEST_COERCED_RATE");
  alternate = first; alternate.width = 2;
  require(!first.matches(alternate), "SELF_TEST_COERCED_SIZE");
  const auto identity = parse_identity_options({L"--endpoint", L"opaque-fixture", L"--max-ms", L"5000", L"--max-parents", L"8"});
  require(identity.endpoint == L"opaque-fixture" && identity.limits.max_parent_nodes == 8, "SELF_TEST_IDENTITY_ARGUMENTS");
  unsigned identity_denied = 0;
  for (const auto& args : std::vector<std::vector<std::wstring>>{
      {L"--endpoint", L"x", L"--max-ms", L"5000"},
      {L"--endpoint", L"x", L"--max-ms", L"5000", L"--max-ms", L"5000"},
      {L"--endpoint", L"x", L"--max-ms", L"5000", L"--open", L"8"},
      {L"--endpoint", L"x", L"--max-ms", L"30001", L"--max-parents", L"8"},
      {L"--endpoint", L"x", L"--max-ms", L"5000", L"--max-parents", L"17"}}) {
    try { parse_identity_options(args); } catch (const std::exception&) { ++identity_denied; }
  }
  require(identity_denied == 5, "SELF_TEST_IDENTITY_CLOSED_ARGUMENTS");
  std::cout << "PASS: 14 hardware-inert camera contract assertions\n";
  return 0;
}

int identity_action(const std::vector<std::wstring>& arguments) {
  const auto options = parse_identity_options(arguments);
  rocell::identity::WindowsIdentityMetadataApi api;
  const auto result = rocell::identity::resolve_identity_metadata(api, options.endpoint, options.limits,
      {[]() { return static_cast<std::uint64_t>(GetTickCount64()); }, []() { return false; }});
  std::cout << rocell::identity::serialize_identity_metadata(result, options.limits) << '\n';
  return 0;
}
} // namespace
...
int wmain(int argc, wchar_t** argv) {
  if (argc == 2 && std::wstring(argv[1]) == L"--self-test") {
    try { return self_test(); } catch (const std::exception& e) { std::cerr << e.what() << '\n'; return 1; }
  }
  if (argc >= 2 && std::wstring(argv[1]) == L"identity") {
    // Explicit metadata-only action. This branch never reaches COM/MF startup
    // below and never constructs a camera source, even when metadata is missing.
    try { return identity_action(std::vector<std::wstring>(argv + 2, argv + argc)); }
    catch (const std::exception&) { std::cerr << "IDENTITY_METADATA_FAILED\n"; return 1; }
  }
#ifdef ROCELL_METADATA_ONLY
  // The dedicated metadata build has no active command surface. Reject before
  // admission parsing, pipe handling, COM/MF startup or camera source creation.
  // This does not change the separately built probe/capture executables.
  if (argc < 2 || std::wstring(argv[1]) != L"inventory") {
    std::cerr << "METADATA_ONLY_OPERATION_REQUIRED\n"; return 2;
  }
#endif
  // This new build cannot bypass parent admission through legacy active argv.
  // The logical Python probe request is translated by the owned adapter; only
  // metadata inventory/identity retain their old directly executable modes.
  std::optional<rocell::admission::AdmissionState> admission;
  std::vector<std::wstring> operation_arguments(argv + 1, argv + argc);
  if (argc >= 2 && std::wstring(argv[1]) == L"--owned-probe") {
    try {
      admission.emplace(rocell::admission::admit_owned_probe(
          std::vector<std::wstring>(argv + 2, argv + argc)));
      operation_arguments = {L"probe", L"--endpoint",
          rocell::admission::endpoint_wide(*admission), L"--max-ms", L"5000"};
    } catch (const std::exception&) {
      std::cerr << "OWNED_ADMISSION_FAILED\n"; return 2;
    }
  }
#ifdef ROCELL_ENABLE_OWNED_CAPTURE
  else if (argc >= 2 && std::wstring(argv[1]) == L"--owned-capture") {
    try {
      admission.emplace(rocell::capture_admission::admit_owned_capture(
          std::vector<std::wstring>(argv + 2, argv + argc)));
      // RELEASE and EOF precede even private-directory inspection. There are
      // no endpoint/output argv bypasses, index fallbacks or directory creation.
      operation_arguments = rocell::capture_admission::capture_arguments(*admission, fs::current_path().wstring());
    } catch (const std::exception&) {
      std::cerr << "OWNED_CAPTURE_ADMISSION_FAILED\n"; return 2;
    }
  }
#endif
  else if (argc < 2 || std::wstring(argv[1]) != L"inventory") {
    std::cerr << "OWNED_ADMISSION_REQUIRED\n"; return 2;
  }
  Receipt receipt;
  ...
  try {
    ...
    if (receipt.options.operation == "capture") {
      require(admission.has_value() && admission->admitted(), "OWNED_CAPTURE_ADMISSION_REQUIRED");
      const auto capture = rocell::capture_admission::parse_capture_settings(admission->request().fields.at("capture_json").text);
    ...
    campaign(receipt, selected, source, reader, callback);
  ...
  const auto result = admission
      ? rocell::admission::wrap_native_result(*admission, receipt.json())
      : receipt.json();
