"""One explicitly admitted r34 smoke movement; no reset, return or retry.
...
def preflight(root, startup_export):
    binding=review_recovery_startup(root,startup_export,revision=34)
    exports=root/'runs/wizard-exports'
    boot=binding['expected_boot']
    claim=exports/f'r34-capture-{boot}.json'
    if claim.exists():
...
def main():
    ...
    mode.add_argument('--authorized-one-movement-clearance-confirmed',action='store_true')
    ...
    with claim.open('x',encoding='utf-8') as stream:
        json.dump(dict(boot=binding['expected_boot'],startup_export=args.startup_export,
                       scope='one-smoke-movement'),stream)
        stream.flush();os.fsync(stream.fileno())
    client=CharacterizationHTTP(binding['address'],key=key,boot=binding['expected_boot'])
    result=SmokeRunner(client,exports,key=key,boot=binding['expected_boot']).run(
