"""One-attempt app-slot-only r94 -> r95 hover install and one startup.
...
from deploy_r94_ghost_b import (APP_SHA as R94_SHA, BACKUP_SHA,
                                FILESYSTEM_SHA)
...
def prepare(root: Path) -> dict:
    ...
    if result["app_sha256"] != APP_SHA:
        raise ValueError("Pinned r95 app differs")
    tools = root / ".firmware-tools"
    path = (tools / "build-configured-diagnostic-candidate-r95--default-4mb-no-psram"
            / "RoArm-M3_example.ino.bin")
    prior_path = (tools / "build-configured-diagnostic-candidate-r94--default-4mb-no-psram"
                  / "RoArm-M3_example.ino.bin")
    image, prior = path.read_bytes(), prior_path.read_bytes()
    sha = lambda data: hashlib.sha256(data).hexdigest()
    if sha(image) != APP_SHA or sha(prior) != R94_SHA or not 0 < len(image) <= 0x140000:
        raise ValueError("Pinned app images or slot bounds differ")
    private = root / "private-backups/controller-20260918-session1"
    rows = [json.loads(line) for line in
            (private / "app-r94-deployment-events.jsonl").read_text().splitlines()]
    if ([row.get("stage") for row in rows] != [
            ...
            rows[0].get("app_sha256") != R94_SHA or
            rows[1].get("mac") != MAC or
            rows[3].get("app_sha256") != R94_SHA or
            rows[3].get("protected_regions_unchanged") is not True):
        raise ValueError("Installed r94 journal differs")
    ...
    journal = private / "app-r95-deployment-events.jsonl"
    if journal.exists():
        raise ValueError("r95 install already attempted; no retry")
...
def install(root: Path, prepared: dict, *, catch_in_place: bool) -> None:
    if catch_in_place is not True:
        raise ValueError("Physical catch must be in place for the single startup")
    root = Path(root).resolve()
    if prepared != prepare(root):
        raise ValueError("Pinned install inputs changed")
...
if __name__ == "__main__":
    raise SystemExit("Invoke prepare(), then install(..., catch_in_place=True) after live checks")
