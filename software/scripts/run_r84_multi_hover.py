"""One signed, finite five-leg noncontact multi-joint hover with per-leg exports."""
import argparse
import json
import os
...
from rocell.application.air_typing_multi_hover_recipe import review_source
from rocell.application.air_typing_r84_campaign import AirTypingMultiHoverHost
...
def preflight(root, startup_export):
    ...
    review_release(root)
    binding = review_recovery_startup(root, startup_export, revision=84)
    exports = root / "runs/wizard-exports"
    source_digest = review_source(exports)
    claim = exports / f'r84-multi-hover-{binding["expected_boot"]}.json'
...
def main(argv=None):
    ...
    if args.preflight_only:
        print(json.dumps(dict(status="R84_MULTI_HOVER_BINDING_VERIFIED",
    ...
    if (status.get("instance_id") != binding["expected_boot"] or status.get("state") != "IDLE"
        ...
        raise ValueError("Reviewed r84 startup no longer idle live boot")
    ...
    with claim.open("x", encoding="utf-8") as stream:
        json.dump(dict(boot=binding["expected_boot"], startup_export=args.startup_export,
                       ...
                       scope="one-fixed-5-leg-noncontact-AIRM5", retry_allowed=False), stream)
        stream.flush(); os.fsync(stream.fileno())
    client = CharacterizationHTTP(binding["address"], key=key,
                                   boot=binding["expected_boot"])
    result = AirTypingMultiHoverHost(client, boot=binding["expected_boot"],
