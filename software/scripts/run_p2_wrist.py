"""One authenticated measured-P2L-to-P2 movement on reviewed r65 startup.
...
def preflight(root,startup_export):
    ...
    binding=review_recovery_startup(root,startup_export,revision=65)
    ...
    replay=assess_large_pose_relief_record(raw,expected_boot=assessment['boot'],profile='P2L')
    if (assessment['status']!='P2L_JOINT_ENDPOINT_MEASURED' or
            assessment['position_delta_counts']!=[0,-65,66,0,0,0,0] or
            canonical(assessment)!=canonical(replay)):
        raise ValueError('Reviewed P2L source evidence differs')
    boot=binding['expected_boot']
    claim=exports/f'r65-large-pose-relief-{boot}.json'
...
def main(argv=None):
    ...
    if args.preflight_only:
        print(json.dumps(dict(status='R65_P2_BINDING_VERIFIED',boot=binding['expected_boot'],
    ...
    if (status.get('instance_id')!=binding['expected_boot'] or
        ...
        raise ValueError('Reviewed r65 startup is no longer the idle live boot')
    ...
    with claim.open('x',encoding='utf-8') as stream:
        json.dump(dict(boot=binding['expected_boot'],startup_export=args.startup_export,
                       ...
                       scope='one-P2L-to-P2-only',retry_allowed=False),stream)
    ...
    result=LargePoseReliefHost(client,export_root=exports,
                               boot=binding['expected_boot'],profile='P2').run_once()
    print(json.dumps(dict(status='P2_RESULT_RECORDED',**result)))
