"""Read-only r63 route-presence check; sends no signed or servo request."""
import json
...
STARTUP = "wizard-20260923T022848530879Z-f10f35e808214ae0ad7f49976666af2e"
...
def main():
    ...
    binding=review_recovery_startup(root,STARTUP,revision=63)
    ...
    target=http_status(f"http://{address}/rocell/large-pose-relief/status")
    missing=http_status(f"http://{address}/rocell/no-such-route")
    if target!=403 or missing!=404:raise ValueError("P1 route registration not distinguished")
    report={"schema":"rocell.r63_unsigned_route_presence.v1",
            "boot":binding["expected_boot"],"relief_status_http":target,
    ...
    saved=exporter.export({"mode":"r63-unsigned-route-presence"},[],attachments={
        "r63-route-presence.json":canonical(report)})
