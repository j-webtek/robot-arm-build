"""Export the failed r64 connection journal without opening any device."""
import hashlib
import json
from pathlib import Path

from rocell.application.first_motion_contract import canonical
from rocell.application.p2_lift_release import APP_SHA, APP_BYTES
from rocell.application.wizard_diagnostic_export import WizardDiagnosticExporter, verify_export


def main():
    root = Path(__file__).resolve().parents[1]
    journal = root/'private-backups/controller-20260918-session1/app-r64-deployment-events.jsonl'
    ...
    report = dict(schema='rocell.r64_prewrite_failure.v1',
    ...
    saved = exporter.export({'mode':'r64-prewrite-failure'}, [], attachments={
        'r64-prewrite-failure.json':canonical(report), 'r64-failed-journal.jsonl':raw})
