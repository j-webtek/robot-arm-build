"""One signed, finite eight-leg noncontact isolated-elbow comparison."""
import argparse
import json
import os
...
from rocell.application.air_typing_elbow_direction_recipe import review_source
from rocell.application.air_typing_r81_campaign import AirTypingElbowDirectionHost
...
def preflight(root,startup_export):
    ...
    binding=review_recovery_startup(root,startup_export,revision=81)
    exports=root/"runs/wizard-exports"
    source_digest=review_source(exports)
    claim=exports/f'r81-elbow-direction-{binding["expected_boot"]}.json'
...
def main(argv=None):
    ...
    if args.preflight_only:
        print(json.dumps(dict(status="R81_ELBOW_DIRECTION_BINDING_VERIFIED",
    ...
    if (status.get("instance_id")!=binding["expected_boot"] or status.get("state")!="IDLE"
        ...
        raise ValueError("Reviewed r81 startup no longer idle live boot")
    ...
    with claim.open("x",encoding="utf-8") as stream:
        json.dump(dict(boot=binding["expected_boot"],startup_export=args.startup_export,
                       ...
                       scope="one-fixed-8-leg-noncontact-AIRE8",retry_allowed=False),stream)
        stream.flush();os.fsync(stream.fileno())
    client=CharacterizationHTTP(binding["address"],key=key,boot=binding["expected_boot"])
    result=AirTypingElbowDirectionHost(client,boot=binding["expected_boot"],
