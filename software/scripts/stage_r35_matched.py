"""Freeze r34 plus bounded matched composition; offline only, never upload."""
import hashlib
...
def main():
    ...
    prefix='.firmware-tools/configured-diagnostic-candidate-r34/RoArm-M3_example/'
    report,receipt=_read(exports,'wizard-20260920T145034031062Z-67bab9148ca244659164801ea1557449',
    ...
    if report['status']!='COMPILED' or set(expected)!=set(files) or any(sha(raw)!=expected[n] for n,raw in files.items()):
        raise ValueError('Pinned r34 source differs')
    before={n:sha(raw) for n,raw in files.items()}
    name='shoulder_characterization_owner.h'
    files[name]=(root/'firmware/diagnostics'/name).read_bytes()
    name='characterization_smoke_board.h'
    old=b'rocell_diag::CharacterizationPattern::Smoke'
    if files[name].count(old)!=1:raise ValueError('Unexpected board composition')
    files[name]=files[name].replace(old,b'rocell_diag::CharacterizationPattern::Matched')
    changes={n:dict(before=before[n],after=sha(raw)) for n,raw in files.items() if before[n]!=sha(raw)}
    if set(changes)!={'shoulder_characterization_owner.h','characterization_smoke_board.h'}:
        raise ValueError('Unexpected change set')
    target=root/'.firmware-tools/configured-diagnostic-candidate-r35/RoArm-M3_example'
    ...
    saved=exporter.export({'mode':'r35-matched-stage'},[],attachments={'matched-stage.json':canonical(dict(
