"""One authenticated measured-T4-to-P4E movement on reviewed r70 startup.
...
def preflight(root,startup_export):
    ...
    binding=review_recovery_startup(root,startup_export,revision=70)
    ...
    if not verify_export(source_dir)['valid']:
        raise ValueError('T4 source export invalid')
    raw=bytes.fromhex((source_dir/'attachment-large-pose-relief.hex.txt').read_text('ascii'))
    replay=assess_large_pose_relief_record(raw,expected_boot=assessment['boot'],profile='T4')
    if (assessment['status']!='T4_JOINT_ENDPOINT_MEASURED' or
            assessment['position_delta_counts']!=[0,0,0,0,62,0,0] or
            canonical(assessment)!=canonical(replay)):
        raise ValueError('Reviewed T4 source evidence differs')
    boot=binding['expected_boot']
    claim=exports/f'r70-large-pose-relief-{boot}.json'
...
def main(argv=None):
    ...
    if args.preflight_only:
        print(json.dumps(dict(status='R70_P4E_BINDING_VERIFIED',boot=binding['expected_boot'],
    ...
    if (status.get('instance_id')!=binding['expected_boot'] or
        ...
        raise ValueError('Reviewed r70 startup is no longer the idle live boot')
    ...
    with claim.open('x',encoding='utf-8') as stream:
        json.dump(dict(boot=binding['expected_boot'],startup_export=args.startup_export,
                       ...
                       scope='one-T4-to-P4E-only',retry_allowed=False),stream)
    ...
    result=LargePoseReliefHost(client,export_root=exports,
                               boot=binding['expected_boot'],profile='P4E').run_once()
    print(json.dumps(dict(status='P4E_RESULT_RECORDED',**result)))
