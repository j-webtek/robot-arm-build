"""One-attempt, app-slot-only r93 -> r94 ghost-B install.
...
if __package__:
    ...
    from .review_r94_ghost_b_candidate import review, COMPILE_ID
...
APP_SHA = "31bceb8f7eb92f220d5221549916d26c4ce0958d269b864600d1dca0d9416c59"
...
def prepare(root: Path) -> dict:
    ...
    if result["app_sha256"] != APP_SHA:
        raise ValueError("Pinned r94 app differs")
    tools = root / ".firmware-tools"
    path = (tools / "build-configured-diagnostic-candidate-r94--default-4mb-no-psram"
            / "RoArm-M3_example.ino.bin")
    prior_path = (tools / "build-configured-diagnostic-candidate-r93--default-4mb-no-psram"
                  / "RoArm-M3_example.ino.bin")
    image, prior = path.read_bytes(), prior_path.read_bytes()
    sha = lambda data: hashlib.sha256(data).hexdigest()
    if sha(image) != APP_SHA or sha(prior) != R93_SHA or not 0 < len(image) <= 0x140000:
        raise ValueError("Pinned app images or slot bounds differ")
    private = root / "private-backups/controller-20260918-session1"
    rows = [json.loads(line) for line in
            (private / "app-r93-deployment-events.jsonl").read_text().splitlines()]
    if ([row.get("stage") for row in rows] != [
            ...
            rows[0].get("app_sha256") != R93_SHA or
            rows[1].get("mac") != MAC or
            rows[3].get("app_sha256") != R93_SHA or
            rows[3].get("protected_regions_unchanged") is not True):
        raise ValueError("Installed r93 journal differs")
    ...
    journal = private / "app-r94-deployment-events.jsonl"
    if journal.exists():
        raise ValueError("r94 install already attempted; no retry")
