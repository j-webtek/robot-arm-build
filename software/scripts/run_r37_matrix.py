"""One explicitly admitted r37 twelve-leg matrix campaign; no reset or retry.
...
def preflight(root, startup_export):
    binding=review_recovery_startup(root,startup_export,revision=37)
    exports=root/'runs/wizard-exports'
    boot=binding['expected_boot']
    claim=exports/f'r37-capture-{boot}.json'
    if claim.exists():
...
def main():
    ...
    with claim.open('x',encoding='utf-8') as stream:
        json.dump(dict(boot=binding['expected_boot'],startup_export=args.startup_export,
                       scope='twelve-leg-matrix-campaign'),stream)
    ...
    result=MatrixRunner(client,exports,key=key,boot=binding['expected_boot'],
