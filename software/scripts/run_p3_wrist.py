"""One authenticated measured-P3E-to-P3 movement on reviewed r67 startup.
...
def preflight(root,startup_export):
    ...
    binding=review_recovery_startup(root,startup_export,revision=67)
    ...
    replay=assess_large_pose_relief_record(raw,expected_boot=assessment['boot'],profile='P3E')
    if (assessment['status']!='P3E_JOINT_ENDPOINT_MEASURED' or
            assessment['position_delta_counts']!=[0,0,0,-64,0,0,0] or
            canonical(assessment)!=canonical(replay)):
        raise ValueError('Reviewed P3E source evidence differs')
    boot=binding['expected_boot']
    claim=exports/f'r67-large-pose-relief-{boot}.json'
...
def main(argv=None):
    ...
    if args.preflight_only:
        print(json.dumps(dict(status='R67_P3_BINDING_VERIFIED',boot=binding['expected_boot'],
    ...
    if (status.get('instance_id')!=binding['expected_boot'] or
        ...
        raise ValueError('Reviewed r67 startup is no longer the idle live boot')
    ...
    with claim.open('x',encoding='utf-8') as stream:
        json.dump(dict(boot=binding['expected_boot'],startup_export=args.startup_export,
                       ...
                       scope='one-P3E-to-P3-only',retry_allowed=False),stream)
    ...
    result=LargePoseReliefHost(client,export_root=exports,
                               boot=binding['expected_boot'],profile='P3').run_once()
    print(json.dumps(dict(status='P3_RESULT_RECORDED',**result)))
