...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='p4-midpoint-simulation-') as folder:
        ...
        subprocess.run([compiler,'-std=c++17','-I'+str(root/'firmware/diagnostics'),
            ...
            str(root/'firmware/diagnostics/test_p4_midpoint_owner.cpp'),'-o',str(executable)],
    ...
    def transport(method,path,body=b''):
        ...
        if action=='start' and method=='POST' and body==b'P4M16':return b'CAPTURING_START'
    ...
    result=P4MidpointHost(transport,boot='ab'*16,export_root=root/'runs/wizard-exports',
    ...
    summary=exporter.export({'mode':'p4-midpoint-simulation'},[],attachments={
        'p4-midpoint-simulation.json':canonical(dict(score=score,exports=result['exports'],
