"""One authenticated measured-T4L-to-T4 movement on reviewed r69 startup.
...
def preflight(root,startup_export):
    ...
    binding=review_recovery_startup(root,startup_export,revision=69)
    ...
    replay=assess_large_pose_relief_record(raw,expected_boot=assessment['boot'],profile='T4L')
    if (assessment['status']!='T4L_JOINT_ENDPOINT_MEASURED' or
            assessment['position_delta_counts']!=[0,-66,65,0,0,0,0] or
            canonical(assessment)!=canonical(replay)):
        raise ValueError('Reviewed T4L source evidence differs')
    boot=binding['expected_boot']
    claim=exports/f'r69-large-pose-relief-{boot}.json'
...
def main(argv=None):
    ...
    if args.preflight_only:
        print(json.dumps(dict(status='R69_T4_BINDING_VERIFIED',boot=binding['expected_boot'],
    ...
    if (status.get('instance_id')!=binding['expected_boot'] or
        ...
        raise ValueError('Reviewed r69 startup is no longer the idle live boot')
    ...
    with claim.open('x',encoding='utf-8') as stream:
        json.dump(dict(boot=binding['expected_boot'],startup_export=args.startup_export,
                       ...
                       scope='one-T4L-to-T4-only',retry_allowed=False),stream)
    ...
    result=LargePoseReliefHost(client,export_root=exports,
                               boot=binding['expected_boot'],profile='T4').run_once()
    print(json.dumps(dict(status='T4_RESULT_RECORDED',**result)))
