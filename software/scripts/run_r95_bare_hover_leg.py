"""Send exactly one attended, no-retry r95 bare-gripper hover leg."""
import hashlib
import http.client
import json
...
from review_r95_bare_hover import APP_SHA
...
def capabilities() -> dict:
    ...
    try:
        connection.request("GET", "/rocell/hover-two-region/capabilities")
        response = connection.getresponse()
        raw = response.read(513)
        if response.status != 200 or len(raw) > 512:
            raise ValueError("Hover identity unavailable")
        data = json.loads(raw)
    finally:
        connection.close()
    ...
    if (type(data) is not dict or set(data) != {
            ...
            data["schema"] != "rocell.bare_gripper_hover.v1" or
        ...
        raise ValueError("Unexpected hover app identity")
    return data
...
def _source(exports: Path, leg: int, boot: str) -> tuple[list[int], list[int]]:
    ...
    marker = exports / f"hover-leg-{leg-1}-{boot}.json"
    completion = exports / f"hover-leg-{leg-1}-final-{boot}.json"
    if not marker.exists() or not completion.exists():
        raise ValueError("Prior leg lacks durable reservation and verified completion")
    ...
    prior = exports / pointer["final_export"]
    if not verify_export(prior)["valid"]:
        raise ValueError("Prior leg export invalid")
    row = json.loads((prior / "attachment-bare-hover-leg.json").read_text())
    if (row.get("category") != "LEG_VERIFIED" or row.get("boot_id") != boot or
            row.get("leg") != leg-1 or row.get("app_sha256") != APP_SHA or
            row.get("after", {}).get("goals") != list(TARGETS[leg-2])):
        raise ValueError("Prior leg evidence differs")
    return row["after"]["positions"], row["after"]["goals"]


def main(*, leg: int, catch_removed_and_path_clear: bool) -> dict:
    if type(leg) is not int or not 1 <= leg <= 5:
        raise ValueError("Expected one of five reviewed hover legs")
    ...
    app = (root / ".firmware-tools/build-configured-diagnostic-candidate-r95--default-4mb-no-psram"
    ...
    if ([row.get("stage") for row in stages] != [
        ...
        raise ValueError("r95 install journal differs")
    caps = capabilities(); boot = caps["boot_id"]
    marker = exports / f"hover-leg-{leg}-{boot}.json"
    if marker.exists():
    ...
    report = dict(schema="rocell.bare_hover_leg_attempt.v1", boot_id=boot,
                  ...
                  physical_key_center_verified=False)

    def save():
        saved = exporter.export({"mode": "r95-bare-gripper-hover-leg"}, [],
            attachments={"bare-hover-leg.json": canonical(report)})
        if not verify_export(Path(saved["path"]))["valid"]:
            raise ValueError("Hover export failed")
        return saved["path"]
    ...
    try:
        ...
        if (before["goals"] != source_goal or
                any(abs(a-b) > tolerance for a,b in zip(before["positions"], source_pos))):
            raise ValueError("Fresh source differs from verified prior pose")
        selected = [i for i in range(7) if TARGETS[leg-1][i] != source_goal[i]]
        if (not selected or 6 in selected or
                max(abs(TARGETS[leg-1][i]-source_goal[i]) for i in selected) > 60):
            raise ValueError("Unreviewed hover target")
        ...
        if (after["goals"] != list(TARGETS[leg-1]) or
            ...
            raise ValueError("Final seven-joint readback differs")
    ...
    if report["category"] == "LEG_VERIFIED":
        publish_reservation_bytes(exports, f"hover-leg-{leg}-final-{boot}.json",
...
if __name__ == "__main__":
    raise SystemExit("Invoke main(leg=N, catch_removed_and_path_clear=True) after live confirmation")
