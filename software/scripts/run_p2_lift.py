"""One authenticated measured-P1-to-P2L movement on reviewed r64 startup.
...
def preflight(root,startup_export):
    ...
    binding=review_recovery_startup(root,startup_export,revision=64)
    ...
    if not verify_export(source_dir)['valid']:
        raise ValueError('P1 source export invalid')
    raw=bytes.fromhex((source_dir/'attachment-large-pose-relief.hex.txt').read_text('ascii'))
    replay=assess_large_pose_relief_record(raw,expected_boot=assessment['boot'])
    if (assessment['status']!='P1_JOINT_ENDPOINT_MEASURED' or
            assessment['position_delta_counts']!=[0,0,0,-60,67,0,0] or
            canonical(assessment)!=canonical(replay)):
        raise ValueError('Reviewed P1 source evidence differs')
    boot=binding['expected_boot']
    claim=exports/f'r64-large-pose-relief-{boot}.json'
...
def main(argv=None):
    ...
    if args.preflight_only:
        print(json.dumps(dict(status='R64_P2L_BINDING_VERIFIED',boot=binding['expected_boot'],
    ...
    if (status.get('instance_id')!=binding['expected_boot'] or
        ...
        raise ValueError('Reviewed r64 startup is no longer the idle live boot')
    ...
    with claim.open('x',encoding='utf-8') as stream:
        json.dump(dict(boot=binding['expected_boot'],startup_export=args.startup_export,
                       ...
                       scope='one-P1-to-P2L-only',retry_allowed=False),stream)
    ...
    result=LargePoseReliefHost(client,export_root=exports,
                               boot=binding['expected_boot'],profile='P2L').run_once()
    print(json.dumps(dict(status='P2L_RESULT_RECORDED',**result)))
