"""One-attempt app-slot-only r95 -> r96 install and one startup.

Never writes filesystem/settings, retries a flash, or sends a motion command.
"""
import hashlib
import io
import json
...
from deploy_r94_ghost_b import BACKUP_SHA, FILESYSTEM_SHA
from deploy_r95_bare_hover import APP_SHA as R95_SHA
from review_r96_registration_ladder import APP_SHA, review
...
def prepare(root: Path) -> dict:
    ...
    if result["app_sha256"] != APP_SHA:
        raise ValueError("Pinned r96 app differs")
    tools = root / ".firmware-tools"
    path = (tools / "build-configured-diagnostic-candidate-r96--default-4mb-no-psram"
            / "RoArm-M3_example.ino.bin")
    prior_path = (tools / "build-configured-diagnostic-candidate-r95--default-4mb-no-psram"
                  / "RoArm-M3_example.ino.bin")
    image, prior = path.read_bytes(), prior_path.read_bytes()
    sha = lambda data: hashlib.sha256(data).hexdigest()
    if sha(image) != APP_SHA or sha(prior) != R95_SHA or not 0 < len(image) <= 0x140000:
        raise ValueError("Pinned r96 images or bounds differ")
    private = root / "private-backups/controller-20260918-session1"
    rows = [json.loads(line) for line in
            (private / "app-r95-deployment-events.jsonl").read_text().splitlines()]
    if ([row.get("stage") for row in rows] != [
            ...
            rows[0].get("app_sha256") != R95_SHA or rows[1].get("mac") != MAC or
            rows[3].get("app_sha256") != R95_SHA or
            rows[3].get("protected_regions_unchanged") is not True):
        raise ValueError("Installed r95 journal differs")
    ...
    journal = private / "app-r96-deployment-events.jsonl"
    if journal.exists():
        raise ValueError("r96 install already attempted; no retry")
...
def install(root: Path, prepared: dict, *, catch_in_place: bool) -> None:
    if catch_in_place is not True:
        raise ValueError("Physical catch required for the single startup")
    root = Path(root).resolve()
    if prepared != prepare(root):
        raise ValueError("Pinned r96 install inputs changed")
    ...
    with prepared["journal"].open("x", encoding="utf-8") as journal:
        ...
        event("RESERVED", app_sha256=APP_SHA, offset=0x10000, bytes=len(prepared["image"]))
        port = serial.Serial(port=None, baudrate=115200, timeout=3, write_timeout=10)
        port.dtr = False; port.rts = False; port.port = PORT
        try:
            port.open(); esp = longer_reset_rom(esptool, port)
            ...
            if (stub.flash_md5sum(0x10000, len(prepared["prior"])) !=
                ...
                raise ValueError("Predecessor or protected region differs")
            ...
            stream = io.BytesIO(prepared["image"]); stream.name = str(prepared["path"])
            args = SimpleNamespace(addr_filename=[(0x10000, stream)], compress=True,
                no_compress=False, no_stub=False, force=False, encrypt=False,
                encrypt_files=None, erase_all=False, verify=False,
                ignore_flash_encryption_efuse_setting=False, flash_size="keep",
                flash_mode="keep", flash_freq="keep")
            event("WRITE_ATTEMPT_STARTED"); cmds.write_flash(stub, args)
            ...
            event("FLASH_VERIFIED", app_sha256=APP_SHA, protected_regions_unchanged=True)
            event("ONE_STARTUP_ATTEMPT"); stub.hard_reset()
...
if __name__ == "__main__":
    raise SystemExit("Invoke prepare(), then install(..., catch_in_place=True) after live confirmation")
