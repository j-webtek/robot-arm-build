"""Stage an exact single-servo P4 wrist candidate from pinned r70, offline."""
import hashlib
...
def specialize(files):
    ...
    changes={
        'large_pose_relief_policy.h':[
            ('T4 -> P4E elbow-only','P4E -> P4 wrist-only'),
            ('source_goals[7]={2047,2217,1897,2777,1915,2040,2047}','source_goals[7]={2047,2217,1897,2711,1915,2040,2047}'),
            ('source_positions[7]={2047,2225,1890,2780,1912,2041,2047}','source_positions[7]={2047,2225,1890,2716,1912,2041,2047}'),
            ('target_goals[7]={2047,2217,1897,2711,1915,2040,2047}','target_goals[7]={2047,2217,1897,2711,1980,2040,2047}'),
            ('selected[1]={3}','selected[1]={4}'),('joint==3','joint==4')],
        'large_pose_relief_owner.h':[
            ('T4 -> P4E elbow extension','P4E -> P4 wrist adjustment'),
            ('LARGE_POSE_P4E_INTENT','LARGE_POSE_P4_INTENT'),
            ('LARGE_POSE_P4E_RECORDED','LARGE_POSE_P4_RECORDED'),
            ('write(uint8_t(14),uint16_t(2711)','write(uint8_t(15),uint16_t(1980)'),
            ('RCP4ELBW01','RCP4WRST01'),('put(2711,2)','put(1980,2)')],
        'large_pose_relief_routes.h':[
            ('T4 -> P4E route','P4E -> P4 route'),('body "P4E"','body "P4"'),
            ('body!="P4E"','body!="P4"'),('LARGE_POSE_P4E_RECORDED','LARGE_POSE_P4_RECORDED')],
        'characterization_board_services.h':[
            ('servo!=14||target!=2711','servo!=15||target!=1980')]
    }
...
def stage(root):
    ...
    path_review=review_p4_wrist(raw,expected_boot='30838e5a53f316c62ac21f976ce95424',
    ...
    if (compiled['status']!='COMPILED' or compiled['target']!='configured-diagnostic-candidate-r70'
        or set(files)!=set(expected) or any(sha(v)!=expected[k] for k,v in files.items())):
        raise ValueError('Pinned r70 source differs')
    candidate=specialize(files)
    target=root/'.firmware-tools/configured-diagnostic-candidate-r71/RoArm-M3_example'
    ...
    saved=exporter.export({'mode':'r71-p4-wrist-stage'},[],attachments={
        'r71-p4-wrist-stage.json':canonical(dict(revision=71,predecessor_compile_receipt=receipt,
            source_export=SOURCE,path_review=path_review,selector='P4',record_bytes=1129,
