"""Export offline eight-leg continuation sweep; no controller connection."""
import json
...
def main():
    ...
    saved = exporter.export({"mode": "r76-continuation-offline-sweep"}, [], attachments={
        "r76-continuation-preview.json": canonical(result)})
    ...
    print(json.dumps(dict(export_path=saved["path"], status=result["status"],
        ...
        min_modeled_separation_mm=min(row["minimum_modeled_link_axis_separation_mm"]
