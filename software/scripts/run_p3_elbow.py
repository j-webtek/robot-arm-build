"""One authenticated measured-P2-to-P3E movement on reviewed r66 startup.
...
def preflight(root,startup_export):
    ...
    binding=review_recovery_startup(root,startup_export,revision=66)
    ...
    if not verify_export(source_dir)['valid']:
        raise ValueError('P2 source export invalid')
    raw=bytes.fromhex((source_dir/'attachment-large-pose-relief.hex.txt').read_text('ascii'))
    replay=assess_large_pose_relief_record(raw,expected_boot=assessment['boot'],profile='P2')
    if (assessment['status']!='P2_JOINT_ENDPOINT_MEASURED' or
            assessment['position_delta_counts']!=[0,0,0,0,64,0,0] or
            canonical(assessment)!=canonical(replay)):
        raise ValueError('Reviewed P2 source evidence differs')
    boot=binding['expected_boot']
    claim=exports/f'r66-large-pose-relief-{boot}.json'
...
def main(argv=None):
    ...
    if args.preflight_only:
        print(json.dumps(dict(status='R66_P3E_BINDING_VERIFIED',boot=binding['expected_boot'],
    ...
    if (status.get('instance_id')!=binding['expected_boot'] or
        ...
        raise ValueError('Reviewed r66 startup is no longer the idle live boot')
    ...
    with claim.open('x',encoding='utf-8') as stream:
        json.dump(dict(boot=binding['expected_boot'],startup_export=args.startup_export,
                       ...
                       scope='one-P2-to-P3E-only',retry_allowed=False),stream)
    ...
    result=LargePoseReliefHost(client,export_root=exports,
                               boot=binding['expected_boot'],profile='P3E').run_once()
    print(json.dumps(dict(status='P3E_RESULT_RECORDED',**result)))
