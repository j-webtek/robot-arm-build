"""Stage exact eight-leg approach-path comparison from compiled r79; no upload."""
import hashlib
...
PREVIEW="wizard-20260925T005512978322Z-a890b3c3a95945e2b9a5eb9846ab5a8e"
...
def specialize(files):
    ...
    policy=replace_one(policy,
        ...
        b"target_goals[7]={1994,2076,2038,2598,2234,2040,2047}")
    files["air_typing_policy.h"]=policy
    files["air_typing_owner.h"]=replace_one(files["air_typing_owner.h"],
                                                 b"RCAIRAB501",b"RCAIRAB601")
    routes=replace_one(files["air_typing_routes.h"],b'body!="AIR6"',b'body!="AIR8"')
    if routes.count(b"/rocell/air-type-repeat/")!=6:
        raise ValueError("Expected six r79 route registrations")
    routes=routes.replace(b"/rocell/air-type-repeat/",b"/rocell/air-type-approach/")
...
def stage(root):
    ...
    sweep,sweep_digest=_read(exports,PREVIEW,"attachment-air-typing-approach-preview.json")
    ...
    if (compiled["status"]!="COMPILED" or compiled["target"]!="configured-diagnostic-candidate-r79"
            ...
            or sweep["status"]!="OFFLINE_APPROACH_SWEEP_PASS_NOT_EXECUTABLE"
        ...
        raise ValueError("Pinned r79 predecessor or approach source evidence differs")
    candidate=specialize(files)
    target=root/".firmware-tools/configured-diagnostic-candidate-r80/RoArm-M3_example"
    target.mkdir(parents=True,exist_ok=True)
    existing={p.name:p.read_bytes() for p in target.iterdir() if p.is_file()}
    if existing and existing!=candidate:
        raise ValueError("Existing r80 stage differs; no overwrite")
    for name,data in candidate.items():
        if not (target/name).exists():
            with (target/name).open("xb") as stream:stream.write(data)
        if (target/name).read_bytes()!=data:raise ValueError("r80 staged readback differs")
    exporter=WizardDiagnosticExporter(exports);exporter.prepare(create=True)
    saved=exporter.export({"mode":"r80-air-typing-approach-stage"},[],attachments={
        "r80-air-typing-approach-stage.json":canonical(dict(
            ...
            selector="AIR8",maximum_writes=8,
