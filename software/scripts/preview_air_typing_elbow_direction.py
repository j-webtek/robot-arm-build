"""Export model-only isolated-elbow sweep from retained r79 A pose."""
import json
from pathlib import Path

from rocell.application.air_typing_elbow_direction_recipe import preview,review_source
...
if __name__=="__main__":
    ...
    saved=exporter.export({"mode":"air-typing-elbow-direction-preview"},[],attachments={
        "air-typing-elbow-direction-preview.json":canonical(report)})
    if not verify_export(Path(saved["path"]))["valid"]:
        raise ValueError("Elbow-direction preview export invalid")
