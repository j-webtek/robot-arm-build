"""Export model-only path comparison from retained r79 A retract; no hardware."""
import json
from pathlib import Path

from rocell.application.air_typing_approach_recipe import preview,review_source
...
if __name__=="__main__":
    ...
    saved=exporter.export({"mode":"air-typing-approach-preview"},[],attachments={
        "air-typing-approach-preview.json":canonical(report)})
    if not verify_export(Path(saved["path"]))["valid"]:
        raise ValueError("Approach preview export invalid")
