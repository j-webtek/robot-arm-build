"""Run the exact r49 separated-probe mapping batch after reviewed startup."""
import argparse
import json
import os
...
REVISION = 49
PLAN_EXPORT = "wizard-20260920T220134625656Z-27b9c57f0b09480fb16b21b26818ea03"
...
def preflight(root, startup_export):
    ...
    saved, _ = _read(exports, PLAN_EXPORT,
                     "attachment-separated-pair-mapping-batch-plan.json")
    expected = plan_separated_mapping_batch()
    if saved != expected:
        raise ValueError("Frozen separated mapping plan binding differs")
...
def main():
    ...
    mode.add_argument("--authorized-fixed-separated-mapping-clearance-confirmed",
                      action="store_true")
    ...
    with claim.open("x", encoding="utf-8") as stream:
        json.dump({"boot": binding["expected_boot"], "startup_export": args.startup_export,
                   "scope": "fixed-separated-pair-mapping-batch",
    ...
    result = SeparatedPairMappingRunner(
        client, exports, key=key, boot=binding["expected_boot"],
