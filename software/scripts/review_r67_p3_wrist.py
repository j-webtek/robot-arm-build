"""Offline r67 source, binary and partition review. No controller access."""
import hashlib
from pathlib import Path

from stage_r67_p3_wrist import specialize
...
def review(root):
    ...
    staged, stage_digest = _read(exports, STAGE, 'attachment-r67-p3-wrist-stage.json')
    sha = lambda data: hashlib.sha256(data).hexdigest()
    prefix = '.firmware-tools/configured-diagnostic-candidate-r67/RoArm-M3_example/'
    old = root/'.firmware-tools/configured-diagnostic-candidate-r66/RoArm-M3_example'
    ...
    if (actual != expected or compiled['status'] != 'COMPILED' or
            compiled['target'] != 'configured-diagnostic-candidate-r67' or
            compiled['build_profile'] != 'default-4mb-no-psram' or
            any(hashes.get(prefix+k) != sha(v) for k,v in actual.items()) or
            staged['selector'] != 'P3' or staged['path_review']['maximum_write_attempts'] != 1 or
            staged['path_review']['retry_allowed'] or staged['path_review']['return_allowed']):
        raise ValueError('r67 source or stage differs')
    build = root/'.firmware-tools/build-configured-diagnostic-candidate-r67--default-4mb-no-psram'
    ...
    if (sha(image) != APP_SHA or len(image) > 0x140000 or
            ...
            not all(s in image for s in (b'RCP3WRST01', b'LARGE_POSE_P3_RECORDED',
                                        b'/rocell/large-pose-relief/start'))):
        raise ValueError('r67 binary differs')
    ...
    report = dict(schema='rocell.r67_p3_wrist_review.v1', app_sha256=APP_SHA,
    ...
    saved = exporter.export({'mode':'r67-p3-wrist-review'}, [], attachments={
        'r67-p3-wrist-review.json':canonical(report)})
